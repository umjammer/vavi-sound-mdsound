...
public class ymz280b extends Instrument
    {
        ...
        {
            return (int)device_start_ymz280b(ChipID, 16934400);
        }

        @Override public int Start(byte ChipID, int clock, int FMClockValue, Object... option)
        {
            return (int)device_start_ymz280b(ChipID, (int)FMClockValue);
        }
 ...
 **********************************************************************************************/
        ...
        //private void ymz280b_update(byte ChipID, int[][] outputs, int samples) { }
        //private int device_start_ymz280b(byte ChipID, int clock) { return 0; }
        //private void device_stop_ymz280b(byte ChipID) { }
        //private void device_reset_ymz280b(byte ChipID) { }

        //private byte ymz280b_r(byte ChipID, int offset) { return 0; }
        //private void ymz280b_w(byte ChipID, int offset, byte data) { }
        //private void ymz280b_write_rom(byte ChipID, int ROMSize, int DataStart, int DataLength, byte[] ROMData) { }

        //private void ymz280b_set_mute_mask(byte ChipID, int MuteMask) { }
...
*/
        ...
        private final int MAX_SAMPLE_CHUNK = 0x10000;
        private final int MAKE_WAVS = 0;
        private final int MAKE_WAVS_CH = 0;

        private final int FRAC_BITS = 14;
        private final int FRAC_ONE = (1 << FRAC_BITS);
        private final int FRAC_MASK = (FRAC_ONE - 1);

        private final int INTERNAL_BUFFER_SIZE = (1 << 15);
        //#define INTERNAL_SAMPLE_RATE	(chip.master_clock * 2.0)
        //private final int INTERNAL_SAMPLE_RATE = chip.rate;
        ...
        public class YMZ280BVoice
        {
            ...
            public int fnum;            /* frequency */
            public byte level;            /* output level */
            public byte pan;              /* panning */

            public int start;           /* start address, in nibbles */
            public int stop;            /* stop address, in nibbles */
            public int loop_start;      /* loop start address, in nibbles */
            public int loop_end;        /* loop end address, in nibbles */
            public int position;        /* current position, in nibbles */

            public int signal;           /* current ADPCM signal */
            public int step;             /* current ADPCM step */

            public int loop_signal;      /* signal at loop start */
            public int loop_step;        /* step at loop start */
            public int loop_count;      /* number of loops so far */

            public int output_left;      /* output volume (left) */
            public int output_right;     /* output volume (right) */
            public int output_step;      /* step value for frequency conversion */
            public int output_pos;       /* current fractional position */
        ...
        public class ymz280b_state
        {
            ...
            public int ext_mem_address_hi;
            public int ext_mem_address_mid;
            public int ext_mem_address;         /* where the CPU can read the ROM */

            public double master_clock;            /* master clock frequency */
            public double rate;
            //void (*irq_callback)(final device_config *, int);		/* IRQ callback */
            public callback irq_callback;       /* IRQ callback */
            public YMZ280BVoice[] voice = new ymz280b.YMZ280BVoice[]{
        ...
        //public void write_to_register(ymz280b_state chip, int data) { }


        /* step size index shift table */
        private int[] index_scale = new int[] { 0x0e6, 0x0e6, 0x0e6, 0x0e6, 0x133, 0x199, 0x200, 0x266 };

        /* lookup table for the precomputed difference */
        private int[] diff_lookup = new int[16];
        ...
        /* TIMER_CALLBACK( update_irq_state_timer_0 );
         ...
         final timer_fired_func update_irq_state_cb[] =
        {
        ...
        private ymz280b_state[] YMZ280BData = new ymz280b_state[] { new ymz280b_state(), new ymz280b_state() };//[MAX_CHIPS];

        @Override public String getName()  { return "YMZ280B"; }  
        @Override public String  getShortName() { return "YMZ"; } 

        /*private ymz280b_state *get_safe_token(final device_config *device)
        {
            assert(device != NULL);
        ...
        private void update_step(ymz280b_state chip, YMZ280BVoice voice)
        {
            ...
            voice.output_step = (int)(frequency * (double)FRAC_ONE / chip.rate);// INTERNAL_SAMPLE_RATE);
        }
        ...
        private byte ymz280b_read_memory(byte[] _base, int size, int offset)
        {
            offset &= 0xFFFFFF;
            if (offset < size)
                return _base[offset];
        ...
        private void update_irq_state_timer_common(ymz280b_state param, int voicenum)
        {
            ymz280b_state chip = (ymz280b_state)param;
        ...
        ***********************************************************************************************/
        ...
        private int generate_adpcm(YMZ280BVoice voice, byte[] _base, int size, short[] buffer, int samples)
        {
            ...
            if (voice.looping == 0)
            {
                ...
                while (samples != 0)
                {
                    ...
                    val = ymz280b_read_memory(_base, size, (int)(position / 2)) >> ((~position & 1) << 2);
        ...
        ***********************************************************************************************/
        ...
        private int generate_pcm8(YMZ280BVoice voice, byte[] _base, int size, short[] buffer, int samples)
        {
            int position = (int)voice.position;
            int val;
            int ptrBuffer = 0;
            ...
            if (voice.looping == 0)
            {
                ...
                while (samples != 0)
                {
                    ...
                    //val = base[position / 2];
                    val = ymz280b_read_memory(_base, size, (int)(position / 2));
        ...
        ***********************************************************************************************/
        ...
        private int generate_pcm16(YMZ280BVoice voice, byte[] _base, int size, short[] buffer, int samples)
        {
            int position = (int)voice.position;
            int val;
            int ptrBuffer = 0;
            ...
            if (voice.looping == 0)
            {
                ...
                while (samples != 0)
                {
                    ...
                    val = (short)((ymz280b_read_memory(_base, size, (int)(position / 2 + 0)) << 8) + ymz280b_read_memory(_base, size, (int)(position / 2 + 1)));
            ...
            else
            {
                ...
                while (samples != 0)
                {
                    /* fetch the current value */
                    //val = (short)((base[position / 2 + 1] << 8) + base[position / 2 + 0]);
                    val = (short)((ymz280b_read_memory(_base, size, (int)(position / 2 + 0)) << 8)
                                + ymz280b_read_memory(_base, size, (int)(position / 2 + 1)));
        ...
        ***********************************************************************************************/
        ...
        public void ymz280b_update(byte ChipID, int[][] outputs, int samples)
        {
            ...
            for (v = 0; v < 8; v++)
            {
                ...
                short prev = voice.last_sample;
                short curr = voice.curr_sample;
                short[] curr_data = chip.scratch;
                int ptrCurr_data = 0;
                int[] ldest = lacc;
                int[] rdest = racc;
                int ptrdest = 0;
                ...
                {
                    int interp_sample = (((int)prev * (FRAC_ONE - voice.output_pos)) + ((int)curr * voice.output_pos)) >> FRAC_BITS;
                ...
                final_pos = (int)(voice.output_pos + remaining * voice.output_step);
                ...
                {
                    case 0x81:
                        samples_left = (int)generate_adpcm(voice, chip.region_base, chip.region_size, chip.scratch, (int)new_samples);
                        break;
                    case 0x82:
                        samples_left = (int)generate_pcm8(voice, chip.region_base, chip.region_size, chip.scratch, (int)new_samples);
                        break;
                    case 0x83:
                        samples_left = (int)generate_pcm16(voice, chip.region_base, chip.region_size, chip.scratch, (int)new_samples);
                        break;
                    default:
                        samples_left = 0;
                ...
                if (samples_left != 0)
                {
                    ...
                    samples_left &= 0xffff;
                    _base = (int)(new_samples - samples_left);
        ...
        ***********************************************************************************************/
        ...
        public int device_start_ymz280b(byte ChipID, int clock)
        {
            ...
            //final ymz280b_interface *intf = (device._config != NULL) ? (final ymz280b_interface *)device._config : &defintrf;
            ...
            /* allocate memory */
            //chip.scratch = auto_alloc_array(device.machine, short, MAX_SAMPLE_CHUNK);
            chip.scratch = new short[MAX_SAMPLE_CHUNK];// malloc(MAX_SAMPLE_CHUNK * sizeof(short));
            for (int i = 0; i < MAX_SAMPLE_CHUNK; i++) chip.scratch[i] = 0x00;
            //memset(chip.scratch, 0x00, MAX_SAMPLE_CHUNK * sizeof(short));
            ...
            return (int)chip.rate;// INTERNAL_SAMPLE_RATE;
        }
        ...
        ***********************************************************************************************/
        ...
        public void write_to_register(ymz280b_state chip, int data)
        {
            ...
            if (chip.current_register < 0x80)
            {
                ...
                switch (chip.current_register & 0xe3)
                {
                    case 0x00:      /* pitch low 8 bits */
                        voice.fnum = (int)((voice.fnum & 0x100) | (data & 0xff));

                        update_step(chip, voice);
                        break;

                    case 0x01:      /* pitch upper 1 bit, loop, key on, mode */
                        voice.fnum = (int)((voice.fnum & 0xff) | ((data & 0x01) << 8));
                    ...
                    case 0x20:      /* start address high */
                        voice.start = (int)((voice.start & (int)(0x00ffff << 1)) | (int)(data << 17));
                        break;

                    case 0x21:      /* loop start address high */
                        voice.loop_start = (int)((voice.loop_start & (int)(0x00ffff << 1)) | (int)(data << 17));
                        break;

                    case 0x22:      /* loop end address high */
                        voice.loop_end = (int)((voice.loop_end & (int)(0x00ffff << 1)) | (int)(data << 17));
                        break;

                    case 0x23:      /* stop address high */
                        voice.stop = (int)((voice.stop & (int)(0x00ffff << 1)) | (int)(data << 17));
                        break;

                    case 0x40:      /* start address middle */
                        voice.start = (int)((voice.start & (int)(0xff00ff << 1)) | (int)(data << 9));
                        break;

                    case 0x41:      /* loop start address middle */
                        voice.loop_start = (int)((voice.loop_start & (int)(0xff00ff << 1)) | (int)(data << 9));
                        break;

                    case 0x42:      /* loop end address middle */
                        voice.loop_end = (int)((voice.loop_end & (int)(0xff00ff << 1)) | (int)(data << 9));
                        break;

                    case 0x43:      /* stop address middle */
                        voice.stop = (int)((voice.stop & (int)(0xff00ff << 1)) | (int)(data << 9));
                        break;

                    case 0x60:      /* start address low */
                        voice.start = (int)((voice.start & (int)(0xffff00 << 1)) | (int)(data << 1));
                        break;

                    case 0x61:      /* loop start address low */
                        voice.loop_start = (int)((voice.loop_start & (int)(0xffff00 << 1)) | (int)(data << 1));
                        break;

                    case 0x62:      /* loop end address low */
                        voice.loop_end = (int)((voice.loop_end & (int)(0xffff00 << 1)) | (int)(data << 1));
                        break;

                    case 0x63:      /* stop address low */
                        voice.stop = (int)((voice.stop & (int)(0xffff00 << 1)) | (int)(data << 1));
                        break;
            ...
            {
                switch (chip.current_register & 0xff)
                {
                    ...
                    case 0x84:      /* ROM readback / RAM write (high) */
                        chip.ext_mem_address_hi = (int)(data << 16);
                        break;

                    case 0x85:      /* ROM readback / RAM write (middle) */
                        chip.ext_mem_address_mid = (int)(data << 8);
                        break;

                    case 0x86:      /* ROM readback / RAM write (low) . update latch */
                        chip.ext_mem_address = (int)(chip.ext_mem_address_hi | chip.ext_mem_address_mid | (int)data);
        ...
        ***********************************************************************************************/
        ...
        private int compute_status(ymz280b_state chip)
        {
            byte result;
        ...
        ***********************************************************************************************/
        ...
        public void ymz280b_write_rom(byte ChipID, int ROMSize, int DataStart, int DataLength, byte[] ROMData)
        {
            ...
            if (chip.region_size != ROMSize)
            {
                ...
                chip.region_size = (int)ROMSize;
                for (int i = 0; i < ROMSize; i++) chip.region_base[i] = (byte) 0xff;
            ...
            for (int i = 0; i < DataLength; i++) chip.region_base[i + DataStart] = ROMData[i];
        ...
        public void ymz280b_write_rom(byte ChipID, int ROMSize, int DataStart, int DataLength, byte[] ROMData, int srcStartAddress)
        {
            ...
            for (int i = 0; i < DataLength; i++) chip.region_base[i + DataStart] = ROMData[i + srcStartAddress];
            //memcpy(chip.region_base + DataStart, ROMData, DataLength);
