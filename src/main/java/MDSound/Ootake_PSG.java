...
public class Ootake_PSG extends Instrument
    {
        /*-----------------------------------------------------------------------------
            [PSG.h]
        ...
        **---------------------------------------------------------------------------*/
        ...
        //private byte PSG_Read(HuC6280 chip, int regNum) { return 0; }

        //private void PSG_Write(HuC6280 chip, int regNum, byte data) { }
        ...
        //Kitao追加
        //private void PSG_SetMutePsgChannel(HuC6280 chip, int num, boolean bMute) { }

        //private void PSG_SetMuteMask(HuC6280 chip, int MuteMask) { }

        //Kitao追加
        //private boolean PSG_GetMutePsgChannel(HuC6280 chip, int num) { return false; }

        //Kitao追加。v2.60
        //private void PSG_SetHoneyInTheSky(HuC6280 chip, boolean bHoneyInTheSky) { }
...
******************************************************************************/
        ...
        public class huc6280_state
        {
            ...
            public int LfoFrq;
            public boolean bLfoOn=false; //v1.59から非使用。過去verのステートロードのために残してある。
            public int LfoCtrl;
            public int LfoShift=0; //v1.59から非使用。過去verのステートロードのために残してある。
            ...
            public boolean[] bPsgMute = new boolean[8];

            public byte[] Port = new byte[16];             // for debug purpose 

            public boolean bWaveCrash; //Kitao追加。DDA再生中にWaveデータが書き換えられたらTRUE
            public boolean bHoneyInTheSky; //はにいいんざすかいパッチ用。v2.60
        }
        ...
        private boolean _bTblInit = false;
        ...
        */
        ...
        {
            (int)8			reg;					// 0-15
            (int)8			data;					// written data
        } ApuQueue;
        typedef struct		//v1.60以前のステートロードのため残してある。
        {
            (int)32			clock;					// cpu cycles elapsed since previous write  Kitao更新。clockは現在非使用。
            (int)8			reg;					// 0-15
            (int)8			data;					// written data
        } OldApuQueue;

        static ApuQueue				_Queue[APUQUEUE_SIZE];
        static (int)32				_QueueWriteIndex;
        static (int)32				_QueueReadIndex;*/
        ...
        private void write_reg(huc6280_state info, byte reg, byte data)
        {
            ...
            switch (reg & 15)
            {
                case 0: // register select
                    info.Channel = (int)(data & 7);
                    break;

                case 1: // main volume
                    info.MainVolumeL = (int)((data >> 4) & 0x0F);
                    info.MainVolumeR = (int)(data & 0x0F);
                ...
                case 2: // frequency low
                    ...
                    if (frq != 0)
                        PSGChn.deltaPhase = (int)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)frq + 0.5); //Kitao更新。速度アップのためfrq以外は定数計算にした。精度向上のため、先に値の小さいOVERSAMPLE_RATEのほうで割るようにした。+0.5は四捨五入で精度アップ。プチノイズ軽減のため。
                    else
                        PSGChn.deltaPhase = 0;
                    break;
                ...
                case 5: // LAL, RAL
                    PSGChn = info.Psg[info.Channel];
                    PSGChn.volumeL = (int)((data >> 4) & 0xF);
                    PSGChn.volumeR = (int)(data & 0xF);
                    PSGChn.outVolumeL = _VolumeTable[PSGChn.volume + (info.MainVolumeL + PSGChn.volumeL) * 2];
                    PSGChn.outVolumeR = _VolumeTable[PSGChn.volume + (info.MainVolumeR + PSGChn.volumeR) * 2];
                    break;
                ...
                case 7: // noise on, noise frq
                    if (info.Channel >= 4)
                    {
                        ...
                        if (PSGChn.noiseFrq == 0)
                            PSGChn.deltaNoisePhase = (int)((double)(2048.0 * info.RESMPL_RATE) + 0.5); //Kitao更新
                        else
                            PSGChn.deltaNoisePhase = (int)((double)(2048.0 * info.RESMPL_RATE) / (double)PSGChn.noiseFrq + 0.5); //Kitao更新
                    }
                    break;
                ...
                case 9: // LFO control  Kitao更新。シンプルに実装してみた。実機で同じ動作かは未確認。はにいいんざすかいの音が似るように実装。v1.59
                    ...
                    info.LfoCtrl = (int)(data & 7); //ドロップロックほらホラで5が使われる。v1.61更新
                    if ((info.LfoCtrl & 4) != 0)
                    ...
                    if ((PSGChn.bOn) && ((i != 1) || (info.LfoCtrl == 0)) && (!info.bPsgMute[i])) //Kitao更新
                    {
                        ...
                        else if (PSGChn.deltaPhase != 0)
                        {
                            ...
                            if ((i == 0) && (info.LfoCtrl > 0))
                            {
                                ...
                                info.Psg[0].phase += (int)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)(info.Psg[0].frq + lfo) + 0.5);
                                info.Psg[1].phase += (int)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)(info.Psg[1].frq * info.LfoFrq) + 0.5); //v1.60更新
                            }
                            else
        ...
        -----------------------------------------------------------------------------*/
        private byte PSG_Read(huc6280_state chip, int regNum)
        {
            huc6280_state info = chip;
        ...
        /*-----------------------------------------------------------------------------
            [Write]
                ＰＳＧポートの書き込みに対する動作を記述します。
        -----------------------------------------------------------------------------*/
        private void PSG_Write(huc6280_state chip, int regNum, byte data)
        {
            huc6280_state info = chip;
            ...
            /*	if (((_QueueWriteIndex + 1) & (APUQUEUE_SIZE-1)) == _QueueReadIndex)
                ...
                _Queue[_QueueWriteIndex].reg   = ((int)8)(regNum & 15);
        ...
        //Kitao追加
        private void PSG_SetMutePsgChannel(huc6280_state chip, int num, boolean bMute)
        {
            huc6280_state info = chip;
        ...
        private void PSG_SetMuteMask(huc6280_state chip, int MuteMask)
        {
            byte CurChn;
        ...
        //Kitao追加
        private boolean PSG_GetMutePsgChannel(huc6280_state chip, int num)
        {
            huc6280_state info = chip;
        ...
        //Kitao追加。v2.60
        private void PSG_SetHoneyInTheSky(huc6280_state chip, boolean bHoneyInTheSky)
        {
            huc6280_state info = chip;
        ...
        private final int DefaultHuC6280ClockValue = 3579545;

        @Override public String getName() { return "HuC6280"; } 
        @Override public String getShortName() { return "HuC8"; } 
        ...
        @Override public int Start(byte ChipID, int SamplingRate, int FMClockValue, Object... Option)
        {
        ...
        public void HuC6280_SetMute(byte ChipID, int val)
        {
            ...
            PSG_SetMuteMask(chip[ChipID], (int)val);
