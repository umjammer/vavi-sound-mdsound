...
**********************************************************************************************/
	...
	public class okim6295 extends Instrument
	{
		...
		{
			return (int)device_start_okim6295(ChipID, (int)clock);
		}
		...
		private final int MAX_SAMPLE_CHUNK = 0x10;  // that's enough for VGMPlay's update rate
		...
		public class ADPCMVoice
		{
			...
			public int base_offset;     /* pointer to the base memory location */
			public int sample;          /* current sample number */
		...
		{
			public final static int OKIM6295_VOICES = 4;
			public ADPCMVoice[] voice = new ADPCMVoice[] { new ADPCMVoice(), new ADPCMVoice(), new ADPCMVoice(), new ADPCMVoice() };//[OKIM6295_VOICES];
			...
			public int master_clock;    /* master clock frequency */
			public int initial_clock;
        ...
        private static int[] index_shift = new int[] { -1, -1, -1, -1, 2, 4, 6, 8 };
		...
		private static int[] volume_table = new int[]
		{
			0x20,	//   0 dB
			0x16,	//  -3.2 dB
			0x10,	//  -6.0 dB
		...
		/* useful interfaces */
		//final okim6295_interface okim6295_interface_pin7high = { 1 };
		//final okim6295_interface okim6295_interface_pin7low = { 0 };
		...
		public okim6295_state[] OKIM6295Data = new okim6295_state[] { new okim6295_state(), new okim6295_state() };
		...
		***********************************************************************************************/
		...
		{
//            System.System.err.printf("nibble={0} diff_lookup[{1}]={2}\n", nibble, state.step * 16 + (nibble & 15), diff_lookup[state.step * 16 + (nibble & 15)]);
//            System.System.err.printf("1state.signal={0}\n", state.signal);
...
//            System.System.err.printf("2state.signal={0}\n", state.signal);
			/* adjust the step size and clamp */
			state.step += index_shift[nibble & 7];
//            System.System.err.printf("3state.signal={0}\n", state.signal);
			if (state.step > 48)
				state.step = 48;
			else if (state.step < 0)
				state.step = 0;

//            System.System.err.printf("4state.signal={0}\n", state.signal);
			/* return the signal */
		...
		***********************************************************************************************/
		...
		private final static int NMK_BNKTBLBITS = 8;
		private final static int NMK_BNKTBLSIZE = 0x100;//(1 << NMK_BNKTBLBITS);  // 0x100
		private final static int NMK_TABLESIZE = (4 * NMK_BNKTBLSIZE);    // 0x400
		private final static int NMK_TABLEMASK = (NMK_TABLESIZE - 1);     // 0x3FF

		private final static int NMK_BANKBITS = 16;
		private final static int NMK_BANKSIZE = 0x10000;//(1 << NMK_BANKBITS);      // 0x10000
		private final static int NMK_BANKMASK = (NMK_BANKSIZE - 1);       // 0xFFFF
		private final int NMK_ROMBASE = (4 * NMK_BANKSIZE);        // 0x40000

        @Override public String getName() { return "OKIM6295"; }
        @Override public String getShortName() { return "OKI9"; }
		...
		private void generate_adpcm(okim6295_state chip, ADPCMVoice voice, short[] buffer, int samples)
		{
			...
			if (voice.playing != 0)
			{
				//System.System.err.printf("base_offset[{0:X}] sample[{1:X}] count[{2:X}]\n", voice.base_offset, voice.sample, voice.count);
		...
		***********************************************************************************************/
		...
		{
			//System.System.err.printf("samples:{0}\n"        , samples);
				...
				if (voice.Muted == 0)
				{
					...
					while (remaining != 0)
					{
						...
						for (samp = 0; samp < Samples; samp++)
						{
                            ...
                            //{
                            //    System.System.err.printf("ch:{0} sampledata[{1}]={2} count:{3} sample:{4}"
		...
		***********************************************************************************************/
		...
		{
			//final okim6295_interface *intf = (final okim6295_interface *)device.baseconfig().static_config;
			//okim6295_state *info = get_safe_token(device);
			okim6295_state info;
			...
			info.bank_offs = 0;
			info.nmk_mode = 0x00;
			//memset(info.nmk_bank, 0x00, 4 * sizeof((int)8));
			for (int i = 0; i < 4; i++)
			{
			...
			info.initial_clock = (int)clock;
			info.master_clock = (int)clock & 0x7FFF_FFFF;
			info.pin7_state = (byte)(((int)clock & 0x80000000) >> 31);
		...
		***********************************************************************************************/
		...
		private static void okim6295_clock_changed(okim6295_state info)
		{
            ...
            info.SmpRateFunc.accept(info.SmpRateData, (int)info.master_clock / divisor);
...
//					System.System.err.printf("OKI6295 start %x contact MAMEDEV\n", temp);
				...
				for (i = 0; i < okim6295_state.OKIM6295_VOICES; i++, temp >>= 1)
				{
					if ((temp & 1) != 0)
					{
						...
						if (start < stop)
						{
							...
							{
								voice.playing = 1;
								voice.base_offset = (int)start;
								voice.sample = 0;
								voice.count = (int)(2 * (stop - start + 1));

								/* also reset the ADPCM parameters */
								reset_adpcm(voice.adpcm);
								voice.volume = (int)volume_table[data & 0x0f];
								Info.keyon[i] = true;
							}
							else
							{
						...
						else
						{
							...
							//System.System.err.printf("OKIM6295: Voice {0}  requested to play invalid sample {1:X2} StartAddr {2:X} StopAdr {3:X} \n", i, info.command, start, stop);
							voice.playing = 0;
						}
					}
				}
		...
		private void okim6295_w(byte ChipID, int offset, byte data)
		{
			...
			switch (offset)
			{
				case 0x00:
					...
					break;
				case 0x08:
					chip.master_clock &= ~((int)0x000000FF);
					chip.master_clock |= (int)(data << 0);
					infos[ChipID].masterClock = chip.master_clock;
					break;
				case 0x09:
					chip.master_clock &= ~((int)0x0000FF00);
					chip.master_clock |= (int)(data << 8);
					infos[ChipID].masterClock = chip.master_clock;
					break;
				case 0x0A:
					chip.master_clock &= ~((int)0x00FF0000);
					chip.master_clock |= (int)(data << 16);
					infos[ChipID].masterClock = chip.master_clock;
					break;
				case 0x0B:
                    data &= 0x7F;
                    chip.master_clock &= ~((int)0xFF000000);
					chip.master_clock |= (int)(data << 24);
		...
		{
			//System.System.err.printf("OKIM6295:okim6295_write_rom2: ChipID:{0} ROMSize:{1:X} DataStart:{2:X} DataLength:{3:X} srcStartAddr:{4:X}\n", ChipID, ROMSize, DataStart, DataLength, srcStartAddr);
			...
			for (int i = 0; i < DataLength; i++)
			{
				...
				//System.err.printf("{0:X02} ", chip.ROM[i + DataStart]);
			}
		...
		private okim6295Info[] infos = new okim6295Info[] { new okim6295Info(), new okim6295Info() };
		...
		public class okim6295Info
		{
			...
			public okim6295ChInfo[] chInfo = new okim6295ChInfo[] { new okim6295ChInfo(), new okim6295ChInfo(), new okim6295ChInfo(), new okim6295ChInfo() };
		}
