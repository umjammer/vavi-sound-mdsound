...
public class c140 extends Instrument
    {
        ...
        private final int MAX_VOICE = 24;
        ...
        public c140_state[] C140Data = new c140_state[] { new c140_state(), new c140_state() };
        ...
        //    //System.System.err.printf("pbase={0:f6} info.baserate={1:d} info.sample_rate={2:d} \n", pbase, info.baserate, info.sample_rate);
        ...
        //    //System.System.err.printf("c140_update");
        ...
        //        {
        //            System.System.err.printf("voicecnt={0:d} ", i);
        //            frequency = info.REG[vreg + 2] * 256 + info.REG[vreg + 3];

        //            /* Abort voice if no frequency value set */
        //            if (frequency == 0) continue;
        //            System.System.err.printf("frequency={0:d} ", frequency);

        //            /* Delta =  frequency * ((8MHz/374)*2 / sample rate) */
        //            delta = (int)((float)frequency * pbase);
        //            System.System.err.printf("delta={0:d} ", delta);

        //            /* Calculate left/right channel volumes */
        //            lvol = (info.REG[vreg + 1] * 32) / MAX_VOICE; //32ch . 24ch
        //            rvol = (info.REG[vreg + 0] * 32) / MAX_VOICE;
        //            System.System.err.printf("MAX_VOICE={0} lvol={1} vreg.volume_left={2} ", MAX_VOICE, lvol, info.REG[vreg + 1]);
        //            System.System.err.printf("rvol={0} vreg.volume_right={1} ", rvol, info.REG[vreg + 0]);
        ...
        //            ed = (int)v.sample_end;
        //            sz = ed - st;
        //            System.System.err.printf("st={0} ed={1} ", st, ed);
        ...
        //            System.System.err.printf("find_sample={0} ", find_sample(info, st, v.bank, i));
        ...
        //            prevdt = (int)v.prevdt;
        //            dltdt = (int)v.dltdt;
        //            System.System.err.printf("offset={0} pos={1} lastdt={2} prevdt={3} dltdt={4} ", offset, pos, lastdt, prevdt, dltdt);

        //            System.System.err.printf("v.mode={0} info.banking_type={1} ", v.mode, (int)info.banking_type);
        ...
        //                    offset &= 0xffff;
        //                    pos += cnt;
        //                    System.System.err.printf("offset={0} cnt={1} pos={2} ", offset, cnt, pos);
        //                    //for(;cnt>0;cnt--)
        //                    {
        ...
        //                        System.System.err.printf("dt={0} ", dt);

        //                        /* decompress to 13bit range */        //2000.06.26 CAB
        //                        sdt = dt >> 3;              //signed
        //                        System.System.err.printf("sdt={0} ", sdt);
        //                        if (sdt < 0) sdt = (sdt << (dt & 7)) - info.pcmtbl[dt & 7];
        //                        else sdt = (sdt << (dt & 7)) + info.pcmtbl[dt & 7];
        //                        System.System.err.printf("sdt={0} info.pcmtbl[dt&7]={1} ", sdt, info.pcmtbl[dt & 7]);

        //                        prevdt = lastdt;
        //                        lastdt = sdt;
        //                        dltdt = (lastdt - prevdt);
        //                        System.System.err.printf("prevdt={0} lastdt={1} dltdt={2} ", prevdt, lastdt, dltdt);
        //                    }
        ...
        //                    lmix[j] += (dt * lvol) >> (5 + 5);
        //                    rmix[j] += (dt * rvol) >> (5 + 5);
        //                    System.System.err.printf("(dt*lvol)>>(5+5)={0} ", (dt * lvol) >> (5 + 5));
        //                    System.System.err.printf("(dt*rvol)>>(5+5)={0} ", (dt * rvol) >> (5 + 5));
        //                }
        //            }
        //            else
        //            {
        ...
        //                    offset &= 0xffff;
        //                    pos += cnt;
        //                    System.System.err.printf("linear offset={0} cnt={1} pos={2} ", offset, cnt, pos);
        //                    /* Check for the end of the sample */
        //                    if (pos >= sz)
        //                    {
        //                        /* Check if its a looping sample, either stop or loop */
        ...
        //                    lmix[j] += (dt * lvol) >> 5;
        //                    rmix[j] += (dt * rvol) >> 5;
        //                    System.System.err.printf("(dt*lvol)>>5={0} ", (dt * lvol) >> 5);
        //                    System.System.err.printf("(dt*rvol)>>5={0} ", (dt * rvol) >> 5);
        //                }
        //            }
        ...
        */
        ...
        private static int[] asic219banks = new int[] { 0x1f7, 0x1f1, 0x1f3, 0x1f5 };
        ...
        public void c140_write_rom(byte ChipID, int ROMSize, int DataStart, int DataLength, byte[] ROMData)
        {
            ...
            if (info.pRomSize != ROMSize)
            {
                ...
                for (int i = 0; i < ROMSize; i++) info.pRom[i] = (byte) 0xff;
        ...
        public void c140_write_rom2(byte ChipID, int ROMSize, int DataStart, int DataLength, byte[] ROMData, int SrcStartAdr)
        {
            ...
            //System.System.err.printf("c140_write_rom2:{0}:{1}:{2}:{3}:{4}", ChipID, ROMSize, DataStart, DataLength, SrcStartAdr);
            return;
        }
        ...
        @Override public String getName () { return "C140"; } 
        @Override public String getShortName () { return "C140"; } 
            ...
            for (i = 0; i < voicecnt; i++)
            {
                ...
                if ((v.mode & 8) != 0 && (info.banking_type != C140_TYPE.ASIC219))
                {
                    ...
                    for (j = 0; j < samples; j++)
                    {
                        ...
                        if (pos >= sz)
                        {
                            //System.err.printf("c140 pos[{0:x}]",pos);
                            //debugCnt = 20;
                            /* Check if its a looping sample, either stop or loop */
                            if ((v.mode & 0x10) != 0)
                            {
                        ...
                        /* Read the chosen sample byte */
                        dt = (byte)info.pRom[(int) (pSampleData + pos)];
                ...
                else
                {
                    ...
                    for (j = 0; j < samples; j++)
                    {
                        ...
                        if (cnt != 0)
                        {
                            ...
                            if (info.banking_type == C140_TYPE.ASIC219)
                            {
                                lastdt = (byte)info.pRom[(int) (pSampleData + (pos ^ 0x01))];
                            ...
                            {
                                lastdt = (byte)info.pRom[(int) (pSampleData + pos)];
                            }
        ...
        {
            //final c140_interface *intf = (final c140_interface *)device.static_config();
            //c140_state *info = get_safe_token(device);
            c140_state info;
            int i;
        ...
        {
            c140_w(ChipID, (int)adr, (byte)data);
            return 0;
        }
    }