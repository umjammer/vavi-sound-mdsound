 ...
 **********************************************************************************************/
    ...
    public class okim6258 extends Instrument
    {
        private final int FOSC_DIV_BY_1024 = 0;
        private final int FOSC_DIV_BY_768 = 1;
        private final int FOSC_DIV_BY_512 = 2;

        private final int TYPE_3BITS = 0;
        private final int TYPE_4BITS = 1;

        private final int OUTPUT_10BITS = 0;
        private final int OUTPUT_12BITS = 1;

        private final int COMMAND_STOP = (1 << 0);
        private final int COMMAND_PLAY = (1 << 1);
        private final int COMMAND_RECORD = (1 << 2);

        private final int STATUS_PLAYING = (1 << 1);
        private final int STATUS_RECORDING = (1 << 2);

        private static int[] dividers=new int[]{ 1024, 768, 512, 512 };

        private final int QUEUE_SIZE = (1 << 1);
        private final int QUEUE_MASK = (QUEUE_SIZE - 1);
        ...
        public okim6258_state[] OKIM6258Data = new okim6258_state[] { new okim6258_state(), new okim6258_state() };
                ...
                for (nib = 0; nib < 16; nib++)
                {
                    diff_lookup[step * 16 + nib] = nbl2bit[nib][0] *
                        (stepval * nbl2bit[nib][1] +
                         ...
                         stepval / 8);
//                    System.System.err.printf("diff_lookup[{0}]={1} ", step * 16 + nib, diff_lookup[step * 16 + nib]);
                }
            }
        ...
        ***********************************************************************************************/
        ...
        @Override public void Update(byte ChipID, int[][] outputs, int samples)
        {
            ...
            if ((chip.status & STATUS_PLAYING)!=0)
            {
                ...
                while (samples!=0)
                {
                    //System.System.err.printf("status={0} chip.nibble_shift={1} ", chip.status, chip.nibble_shift);
                    /* Compute the new amplitude and update the current step */
                    //int nibble = (chip.data_in >> nibble_shift) & 0xf;
                    int nibble;
                    short sample;

                    //System.System.err.printf("chip.data_empty={0} ", chip.data_empty);
                    if (nibble_shift==0)
                    {
                        // 1st nibble - get data
                    ...
                    //*buffer++ = sample;
                    //System.System.err.printf("chip.pan={0} sample={1} ", chip.pan, sample);
...
//                    System.System.err.printf("passed ");
        ...
        ***********************************************************************************************/
        ...
        {
            //System.System.err.printf("device_start_okim6258 ChipID{0} clock{1} divider{2} adpcm_type{3} output_12bits{4} ", ChipID, clock, divider, adpcm_type, output_12bits);
            //final okim6258_interface *intf = (final okim6258_interface *)device.baseconfig().static_config();
            ...
            if (Iternal10Bit!=0)
                info.output_mask = (int)(1 << (info.output_bits - 1));
            else
                info.output_mask = (1 << (12 - 1));
            info.divider = (int)dividers[/*intf.*/divider];
            ...
            //System.System.err.printf("get_vclk(info)={0} ", get_vclk(info));
        ...
        ***********************************************************************************************/
        ...
        private void device_reset_okim6258(byte ChipID)
        {
            ...
            info.divider = (int)dividers[info.initial_div];
            if (info.SmpRateFunc != null)
            {
                info.SmpRateFunc.accept(info.SmpRateData, get_vclk(info));
        ...
        ***********************************************************************************************/
        ...
        private void okim6258_set_clock(byte ChipID, int val)
        {
            ...
            if (val!=0)
            {
                info.master_clock = (int)val;
            }
            else
            {
                info.master_clock = (int)((info.clock_buffer[0x00] << 0) |
        ...
        ***********************************************************************************************/
        ...
        /*(int)8 okim6258_status_r((int)8 ChipID, offs_t offset)
        {
            //okim6258_state *info = get_safe_token(device);
            okim6258_state *info = &OKIM6258Data[ChipID];
        ...
        ***********************************************************************************************/
        ...
        @Override public int Start(byte ChipID,int samplingrate, int clock,Object... option) {
            ...
            return (int)device_start_okim6258(ChipID, clock, divider, adpcm_type, output_12bits);
        }

        @Override public String getName() { return "OKIM6258"; } 
        @Override public String getShortName() { return "OKI5"; } 
        ...
        {
            //System.System.err.printf("port={0:X2} data={1:X2} \n", Port, Data);
            switch (Port)
            {
                case 0x00:
