...
import java.util.List;
    ...
    public class sn76489 extends Instrument
    {
        ...
        private final double SQRT2 = 1.414213562;
        private final double RANGE = 512;
        private final int DefaultPSGClockValue = 3579545;

        private final int NoiseInitialState = 0x8000;  /* Initial state of shift register */
        private final int PSG_CUTOFF = 0x6;     /* Value below which PSG does not output */

        private final List<Integer> PSGVolumeValues = Arrays.asList(
        ...
        public SN76489_Context[] SN76489_Chip = new SN76489_Context[] { new SN76489_Context(), new SN76489_Context() };
        ...
        public void SN76489_GGStereoWrite(byte ChipID, int data)
        {
            ...
            chip.PSGStereo = data;
            //System.err.printf("WrPSGStereo:0:{0}", SN76489_Chip[0].PSGStereo);
            //System.err.printf("WrPSGStereo:1:{0}", SN76489_Chip[1].PSGStereo);
        }
        ...
        @Override public String getName()  { return "SN76489"; } 
        @Override public String getShortName()  { return "DCSG"; } 
        ...
        @Override public int Start(byte ChipID, int SamplingRate, int PSGClockValue,Object... option)
        {
            int i;
        ...
        @Override public void Reset(byte ChipID)
        {
            ...
            for (i = 0; i <= 3; i++)
            {
                ...
                chip.IntermediatePos[i] = Float.MIN_VALUE;
        ...
        @Override public void Update(byte ChipID, int[][] buffer, int length)
        {
            SN76489_Context chip = SN76489_Chip[ChipID];
            ...
            for (j = 0; j < length; j++)
            {
                ...
                for (i = 0; i <= 2; ++i)
                    ...
                    {
                        if (chip_t.IntermediatePos[i] != Float.MIN_VALUE)
                            /* Intermediate position (antialiasing) */
                            chip.Channels[i] = (short)(PSGVolumeValues.get(chip.Registers[2 * i + 1]) * chip_t.IntermediatePos[i]);
                        else
                            /* Flat (no antialiasing needed) */
                            chip.Channels[i] = PSGVolumeValues.get(chip.Registers[2 * i + 1]) * chip_t.ToneFreqPos[i];
                    }
                    else
                        /* Muted channel */
                        chip.Channels[i] = 0;
                ...
                if ((chip_n.Mute >> 3 & 1) != 0)
                {
                    ...
                    chip.Channels[3] = PSGVolumeValues.get(chip.Registers[7]) * ((chip_n.NoiseShiftRegister & 0x1) * 2 - 1);
                ...
                if (chip.NgpFlags == 0)
                {
                    ...
                    for (i = 0; i <= 3; ++i)
                    {
                        ...
                        else
                        {
                            ...
                            //System.err.printf("Ch:bl:br:{0}:{1}:{2}:{3}",i,bl,br, chip.Channels[i]);
                        }
                ...
                if (chip.ToneFreqVals[3] <= 0)
                {
                    ...
                    if (chip.ToneFreqPos[3] == 1)
                    {
                        ...
                        if ((chip.Registers[6] & 0x4) !=0)
                        {
                            ...
                            switch (chip.WhiteNoiseFeedback)
                            {
                                ...
                                case FB_SEGAVDP: /* SMS, GG, MD  %00001001 */
                                    ...
                                    Feedback = chip.NoiseShiftRegister & (int)chip.WhiteNoiseFeedback.ordinal();
                                    Feedback = (Feedback > 0) && (((chip.NoiseShiftRegister & (int)chip.WhiteNoiseFeedback.ordinal()) ^ (int)chip.WhiteNoiseFeedback.ordinal()) > 0) ? 1 : 0;
                                    break;
                                default:
                        ...
                        chip.NoiseShiftRegister = (chip.NoiseShiftRegister >> 1) | (Feedback << ((int)chip.SRWidth.ordinal() - 1));
                    }
                }
            }
