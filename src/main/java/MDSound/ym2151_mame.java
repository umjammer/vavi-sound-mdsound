...
public class ym2151_mame extends Instrument
    {
        private final int DefaultYM2151ClockValue = 3579545;

        @Override public String getName ()  { return "YM2151mame"; } 
        @Override public String getShortName() { return "OPMm"; } 
        ...
        @Override public int Start(byte ChipID, int sampleRate, int ClockValue, Object... option)
        {
            ym2151_state info;
        ...
        private ym2151_state[] YM2151Data = new ym2151_state[] { new ym2151_state(), new ym2151_state() };
...
// #endif
            ...
            */
            ...
            //void (*irqhandler)(final device_config *device, int irq);		/* IRQ function handler */
        ...
        private final int LFO_SH = 10;  /* 22.10 fixed point (LFO calculations)       */
        ...
        private final int TL_RES_LEN = (256); /* 8 bits addressing (real chip) */
        ...
        */
        private final int TL_TAB_LEN = (13 * 2 * TL_RES_LEN);
...
/* DT1=3 */
  ...
  8, 8, 9,10,11,12,13,14,16,17,19,20,22,22,22,22
        ...
        */
        ...
        private byte[] lfo_noise_waveform = new byte[] {
...
// #endif
                ...
                chip.freq[768 + 2 * 768 + i] = (int)(((int)(phaseinc * mult)) & 0xffffffc0); /* adjust to X.10 fixed point */
...
// #endif
        ...
        private void set_connect(YM2151Operator[] om1Buf, int om1Ptr, int cha, int v)
        {
            ...
            //System.err.printf("v:{0} c1:{1} mem:{2} c2:{3} m2:{4} chanout[cha]:{5}"
                ...
                case 0x00:
                    switch (r)
                    {
                        ...
                        case 0x10:  /* timer A hi */
                            chip.timer_A_index = (chip.timer_A_index & 0x003) | (int)(v << 2);
                            break;

                        case 0x11:  /* timer A low */
                            chip.timer_A_index = (chip.timer_A_index & 0x3fc) | (int)(v & 3);
                            break;

                        case 0x12:  /* timer B */
                            chip.timer_B_index = (int)v;
                            break;

                        case 0x14:  /* CSM, irq flag reset, irq enable, timer start/stop */

                            chip.irq_enable = (int)v;   /* bit 3-timer B, bit 2-timer A, bit 7 - CSM */
...
// #endif
                        ...
                        case 0x18:  /* LFO frequency */
                            {
                                chip.lfo_overflow = (int)((1 << ((15 - (v >> 4)) + 3)) * (1 << LFO_SH));
                                chip.lfo_counter_add = (int)(0x10 + (v & 0x0f));
                            }
                            break;
...
// #endif
                ...
                case 0x20:
                    ...
                    switch (r & 0x18)
                    {
                        case 0x00:  /* RL enable, Feedback, Connection */
                            op.fb_shift = (int)(((v >> 3) & 7) != 0 ? ((v >> 3) & 7) + 6 : 0);
                            chip.pan[(r & 7) * 2] = (int)((v & 0x40) != 0 ? ~0 : 0);
                            chip.pan[(r & 7) * 2 + 1] = (int)((v & 0x80) != 0 ? ~0 : 0);
                        ...
                        case 0x08:  /* Key Code */
                            ...
                            if (v != op.kc)
                            {
                                ...
                                kc_channel = (int)((v - (v >> 2)) * 64);
                                kc_channel += 768;
                                kc_channel |= (op.kc_i & 63);

                                opBuf[opPtr + 0].kc = (int)v;
                                opBuf[opPtr + 0].kc_i = kc_channel;
                                opBuf[opPtr + 1].kc = (int)v;
                                opBuf[opPtr + 1].kc_i = kc_channel;
                                opBuf[opPtr + 2].kc = (int)v;
                                opBuf[opPtr + 2].kc_i = kc_channel;
                                opBuf[opPtr + 3].kc = (int)v;
                                opBuf[opPtr + 3].kc_i = kc_channel;

                                kc = (int)(v >> 2);

                                opBuf[opPtr + 0].dt1 = chip.dt1_freq[opBuf[opPtr + 0].dt1_i + kc];
                                opBuf[opPtr + 0].freq = (int)(((chip.freq[kc_channel + opBuf[opPtr + 0].dt2] + opBuf[opPtr + 0].dt1) * opBuf[opPtr + 0].mul) >> 1);

                                opBuf[opPtr + 1].dt1 = chip.dt1_freq[opBuf[opPtr + 1].dt1_i + kc];
                                opBuf[opPtr + 1].freq = (int)(((chip.freq[kc_channel + opBuf[opPtr + 1].dt2] + opBuf[opPtr + 1].dt1) * opBuf[opPtr + 1].mul) >> 1);

                                opBuf[opPtr + 2].dt1 = chip.dt1_freq[opBuf[opPtr + 2].dt1_i + kc];
                                opBuf[opPtr + 2].freq = (int)(((chip.freq[kc_channel + opBuf[opPtr + 2].dt2] + opBuf[opPtr + 2].dt1) * opBuf[opPtr + 2].mul) >> 1);

                                opBuf[opPtr + 3].dt1 = chip.dt1_freq[opBuf[opPtr + 3].dt1_i + kc];
                                opBuf[opPtr + 3].freq = (int)(((chip.freq[kc_channel + opBuf[opPtr + 3].dt2] + opBuf[opPtr + 3].dt1) * opBuf[opPtr + 3].mul) >> 1);
                ...
                case 0x40:      /* DT1, MUL */
                    {
                        ...
                        op.dt1_i = (int)((v & 0x70) << 1);
                        op.mul = (int)((v & 0x0f) != 0 ? (v & 0x0f) << 1 : 1);

                        if (olddt1_i != op.dt1_i)
                            op.dt1 = chip.dt1_freq[op.dt1_i + (op.kc >> 2)];

                        if ((olddt1_i != op.dt1_i) || (oldmul != op.mul))
                            op.freq = (int)(((chip.freq[op.kc_i + op.dt2] + op.dt1) * op.mul) >> 1);
                    }
                    break;

                case 0x60:      /* TL */
                    op.tl = (int)((v & 0x7f) << (ENV_BITS - 7)); /* 7bit TL */
                    break;
                ...
                case 0x80:      /* KS, AR */
                    {
                        ...
                        op.ks = (int)(5 - (v >> 6));
                        op.ar = (int)((v & 0x1f) != 0 ? 32 + ((v & 0x1f) << 1) : 0);
                ...
                case 0xa0:      /* LFO AM enable, D1R */
                    op.AMmask = (int)((v & 0x80) != 0 ? ~0 : 0);
                    op.d1r = (int)((v & 0x1f) != 0 ? 32 + ((v & 0x1f) << 1) : 0);
                    op.eg_sh_d1r = eg_rate_shift[op.d1r + (op.kc >> (int)op.ks)];
                    op.eg_sel_d1r = eg_rate_select[op.d1r + (op.kc >> (int)op.ks)];
                    break;
                ...
                case 0xc0:      /* DT2, D2R */
                    ...
                    op.d2r = (int)((v & 0x1f) != 0 ? 32 + ((v & 0x1f) << 1) : 0);
                    op.eg_sh_d2r = eg_rate_shift[op.d2r + (op.kc >> (int)op.ks)];
                    op.eg_sel_d2r = eg_rate_select[op.d2r + (op.kc >> (int)op.ks)];
                    break;
...
// #if true // disabled for now due to crashing with winalloc.c (ERROR_NOT_ENOUGH_MEMORY)
        ...
        */
        ...
        static void ym2151_state_save_register( YM2151 *chip, final device_config *device )
        {
            int j;
...
// #endif
        ...
        */
        private YM2151 ym2151_init(int clock, int rate)
        {
            ...
            PSG.clock = (int)clock;
            /*rate = clock/64;*/
            PSG.sampfreq = (int)(rate != 0 ? rate : 44100);    /* avoid division by 0 in init_chip_tables() */
...
// #endif
        ...
        private int op_calc(YM2151Operator OP, int env, int pm)
        {
            int p;
        ...
        private int op_calc1(YM2151Operator OP, int env, int pm)
        {
            ...
            //System.err.printf("p:{0} tl_tab[p]:{1}", p, tl_tab[p]);
            return tl_tab[p];
        }



        private int volume_calc(YM2151Operator OP, int AM)
        {
            return ((OP).tl + ((int)(OP).volume) + (AM & (OP).AMmask));
        }
        ...
        private void chan_calc(int chan)
        {
            ...
            //if (chan == 0)
            //{
                //System.err.printf("Ch:{0} ENV_QUIET:{1} op.tl:{2} op.volume:{3} op.state:{4} PSG.eg_cnt:{5} PSG.eg_timer_add:{6} PSG.eg_timer_overflow:{7} \n"
                //, chan, ENV_QUIET, op.tl, op.volume, op.state, PSG.eg_cnt, PSG.eg_timer_add, PSG.eg_timer_overflow);
            //}
            env = volume_calc(op, AM);
            {
                int out_ = op.fb_out_prev + op.fb_out_curr;
            ...
            if (env < ENV_QUIET)
            {
                ...
                //System.err.printf("chanout[chan]:{0} env:{1} c2:{2}", chanout[chan].v, env, c2.v);
            }
        ...
        private void chan7_calc()
        {
            ...
            if (op.ams != 0)
                AM = PSG.lfa << (int)(op.ams - 1);
            env = volume_calc(op, AM);
            //System.err.printf("1:env:{0} ENV_QUIET:{1} op.tl:{2} op.volume:{3} op.state:{4} PSG.eg_cnt:{5} PSG.eg_timer_add:{6} PSG.eg_timer_overflow:{7} \n"
                //, env, ENV_QUIET, op.tl, op.volume, op.state, PSG.eg_cnt, PSG.eg_timer_add, PSG.eg_timer_overflow);
            {
                ...
                if (env < ENV_QUIET)
                {
                    if (op.fb_shift == 0)
                        out_ = 0;
                    op.fb_out_curr = op_calc1(op, env, (out_ << (int)op.fb_shift));
                    //System.err.printf("2:env:{0} ENV_QUIET:{1} \n", env, ENV_QUIET);
                }
            }

            env = volume_calc(opBuf[opPtr + 1], AM);  /* M2 */
            //System.err.printf("3:env:{0} ENV_QUIET:{1} \n", env, ENV_QUIET);
            if (env < ENV_QUIET)
                opBuf[opPtr + 1].connect.v += op_calc(opBuf[opPtr + 1], env, m2.v);

            env = volume_calc(opBuf[opPtr + 2], AM);  /* C1 */
            //System.err.printf("4:env:{0} ENV_QUIET:{1} \n", env, ENV_QUIET);
            if (env < ENV_QUIET)
                opBuf[opPtr + 2].connect.v += op_calc(opBuf[opPtr + 2], env, c1.v);

            env = volume_calc(opBuf[opPtr + 3], AM);  /* C2 */
            //System.err.printf("5:env:{0} ENV_QUIET:{1} \n", env, ENV_QUIET);
            if ((PSG.noise & 0x80) != 0)
            {
                int noiseout;
            ...
            */
            ...
            do
            {
                if (op.pms != 0)    /* only when phase modulation from LFO is enabled for this channel */
                {
                    ...
                    if (mod_ind != 0)
                    {
                        int kc_channel = (int)(op.kc_i + mod_ind);
                        opBuf[opPtr + 0].phase += (int)(((PSG.freq[kc_channel + opBuf[opPtr + 0].dt2] + opBuf[opPtr + 0].dt1) * opBuf[opPtr + 0].mul) >> 1);
                        opBuf[opPtr + 1].phase += (int)(((PSG.freq[kc_channel + opBuf[opPtr + 1].dt2] + opBuf[opPtr + 1].dt1) * opBuf[opPtr + 1].mul) >> 1);
                        opBuf[opPtr + 2].phase += (int)(((PSG.freq[kc_channel + opBuf[opPtr + 2].dt2] + opBuf[opPtr + 2].dt1) * opBuf[opPtr + 2].mul) >> 1);
                        opBuf[opPtr + 3].phase += (int)(((PSG.freq[kc_channel + opBuf[opPtr + 3].dt2] + opBuf[opPtr + 3].dt1) * opBuf[opPtr + 3].mul) >> 1);
                    }
...
// #endif
            ...
            for (i = 0; i < length; i++)
            {
                ...
                //System.err.printf("{0} {1}", outl, outr);
                bufL[i] = (short)outl;
                bufR[i] = (short)outr;
