package MDSound;
    ...
    public class pokey extends Instrument
    {
        @Override public String getName() { return "POKEY"; }
        @Override public String getShortName() { return "POKEY"; }
        ...
        {
            return (int)device_start_pokey(ChipID, (int)1789772);
        }

        @Override public int Start(byte ChipID, int clock, int ClockValue, Object... option)
        {
            return (int)device_start_pokey(ChipID, (int)ClockValue);
        }
 ...
 *****************************************************************************/
        ...
        private final int AUDF1_C = 0x00;
        private final int AUDC1_C = 0x01;
        private final int AUDF2_C = 0x02;
        private final int AUDC2_C = 0x03;
        private final int AUDF3_C = 0x04;
        private final int AUDC3_C = 0x05;
        private final int AUDF4_C = 0x06;
        private final int AUDC4_C = 0x07;
        private final int AUDCTL_C = 0x08;
        private final int STIMER_C = 0x09;
        private final int SKREST_C = 0x0A;
        private final int POTGO_C = 0x0B;
        private final int SEROUT_C = 0x0D;
        private final int IRQEN_C = 0x0E;
        private final int SKCTL_C = 0x0F;
        /* POKEY READ LOGICALS */
        private final int POT0_C = 0x00;
        private final int POT1_C = 0x01;
        private final int POT2_C = 0x02;
        private final int POT3_C = 0x03;
        private final int POT4_C = 0x04;
        private final int POT5_C = 0x05;
        private final int POT6_C = 0x06;
        private final int POT7_C = 0x07;
        private final int ALLPOT_C = 0x08;
        private final int KBCODE_C = 0x09;
        private final int RANDOM_C = 0x0A;
        private final int SERIN_C = 0x0D;
        private final int IRQST_C = 0x0E;
        private final int SKSTAT_C = 0x0F;
        /* exact 1.79 MHz clock freq (of the Atari 800 that is) */
        private final int FREQ_17_EXACT = 1789790;
        ...
        //void pokey_update((int)8 ChipID, stream_sample_t** outputs, int samples);
        //int device_start_pokey((int)8 ChipID, int clock);
        //void device_stop_pokey((int)8 ChipID);
        //void device_reset_pokey((int)8 ChipID);

        ////READ8_DEVICE_HANDLER( pokey_r );
        ////WRITE8_DEVICE_HANDLER( pokey_w );
        //(int)8 pokey_r((int)8 ChipID, offs_t offset);
        //void pokey_w((int)8 ChipID, offs_t offset, (int)8 data);
        ...
        //void pokey_set_mute_mask((int)8 ChipID, (int)32 MuteMask);
        ...
        private final int HEAVY_MACRO_USAGE = 1;
        //#endif
        //#else
        //#define HEAVY_MACRO_USAGE	BIG_SWITCH
        //#endif

        private final int SUPPRESS_INAUDIBLE = 1;
         ...
         */
        private final int POKEY_DEFAULT_GAIN = (32767 / 11 / 4);
        private final int VERBOSE = 0;
        private final int VERBOSE_SOUND = 0;
        private final int VERBOSE_TIMER = 0;
        private final int VERBOSE_POLY = 0;
        private final int VERBOSE_RAND = 0;
        ...
        private final int CHAN1 = 0;
        private final int CHAN2 = 1;
        private final int CHAN3 = 2;
        private final int CHAN4 = 3;

        private final int TIMER1 = 0;
        private final int TIMER2 = 1;
        private final int TIMER4 = 2;

        /* values to add to the divisors for the different modes */
        private final int DIVADD_LOCLK = 1;
        private final int DIVADD_HICLK = 4;
        private final int DIVADD_HICLK_JOINED = 7;

        /* AUDCx */
        private final int NOTPOLY5 = 0x80;  /* selects POLY5 or direct CLOCK */
        private final int POLY4 = 0x40; /* selects POLY4 or POLY17 */
        private final int PURE = 0x20;  /* selects POLY4/17 or PURE tone */
        private final int VOLUME_ONLY = 0x10;   /* selects VOLUME OUTPUT ONLY */
        private final int VOLUME_MASK = 0x0f;  /* volume mask */

        /* AUDCTL */
        private final int POLY9 = 0x80;	/* selects POLY9 or POLY17 */
        private final int CH1_HICLK = 0x40;	/* selects 1.78979 MHz for Ch 1 */
        private final int CH3_HICLK = 0x20;	/* selects 1.78979 MHz for Ch 3 */
        private final int CH12_JOINED = 0x10;	/* clocks channel 1 w/channel 2 */
        private final int CH34_JOINED = 0x08;	/* clocks channel 3 w/channel 4 */
        private final int CH1_FILTER = 0x04;	/* selects channel 1 high pass filter */
        private final int CH2_FILTER = 0x02;	/* selects channel 2 high pass filter */
        private final int CLK_15KHZ = 0x01; /* selects 15.6999 kHz or 63.9211 kHz */

        /* IRQEN (D20E) */
        private final int IRQ_BREAK = 0x80;	/* BREAK key pressed interrupt */
        private final int IRQ_KEYBD = 0x40;	/* keyboard data ready interrupt */
        private final int IRQ_SERIN = 0x20;	/* serial input data ready interrupt */
        private final int IRQ_SEROR = 0x10;	/* serial output register ready interrupt */
        private final int IRQ_SEROC = 0x08;	/* serial output complete interrupt */
        private final int IRQ_TIMR4 = 0x04;	/* timer channel // #4 interrupt */
        private final int IRQ_TIMR2 = 0x02;	/* timer channel // #2 interrupt */
        private final int IRQ_TIMR1 = 0x01;	/* timer channel // #1 interrupt */

        /* SKSTAT (R/D20F) */
        private final int SK_FRAME = 0x80;	/* serial framing error */
        private final int SK_OVERRUN = 0x40;	/* serial overrun error */
        private final int SK_KBERR = 0x20;	/* keyboard overrun error */
        private final int SK_SERIN = 0x10;	/* serial input high */
        private final int SK_SHIFT = 0x08;	/* shift key pressed */
        private final int SK_KEYBD = 0x04;	/* keyboard key pressed */
        private final int SK_SEROUT = 0x02;	/* serial output active */

        /* SKCTL (W/D20F) */
        private final int SK_BREAK = 0x80;	/* serial out break signal */
        private final int SK_BPS = 0x70;	/* bits per second */
        private final int SK_FM = 0x08;	/* FM mode */
        private final int SK_PADDLE = 0x04;	/* fast paddle a/d conversion */
        private final int SK_RESET = 0x03;	/* reset serial/keyboard interface */

        private final int DIV_64 = 28;		 /* divisor for 1.78979 MHz clock to 63.9211 kHz */
        private final int DIV_15 = 114; 	 /* divisor for 1.78979 MHz clock to 15.6999 kHz */
        ...
        private void ADJUST_EVENT(pokey_state chip, int _event)
        {
            chip.counter[CHAN1] -= (int)_event;
            chip.counter[CHAN2] -= (int)_event;
            chip.counter[CHAN3] -= (int)_event;
            chip.counter[CHAN4] -= (int)_event;
            chip.samplepos_whole -= (int)_event;
            chip.polyadjust += (int)_event;
        }

        //#if SUPPRESS_INAUDIBLE

        private void PROCESS_CHANNEL(pokey_state chip, int ch, int _event, int sum)
        {
            int toggle = 0;
        ...
        private void PROCESS_SAMPLE(pokey_state chip, int samples, int[] bufL, int[] bufR, int bufPtr, int _event, int sum)
        {
            ADJUST_EVENT(chip, _event);
        ...
        private void PROCESS_POKEY(pokey_state chip, int samples, int[] bufL, int[] bufR, int bufPtr)
        {
            ...
            while (samples > 0)
            {
                if (chip.counter[CHAN1] < chip.samplepos_whole)
                {
                    if (chip.counter[CHAN2] < chip.counter[CHAN1])
                    {
                        if (chip.counter[CHAN3] < chip.counter[CHAN2])
                        {
                            ...
                            {
                                int _event = (int)chip.counter[CHAN4];
                                PROCESS_CHANNEL(chip, CHAN4, _event, sum);
                            }
                            else
                            {
                                int _event = (int)chip.counter[CHAN3];
                                PROCESS_CHANNEL(chip, CHAN3, _event, sum);
                            }
                        }
                        else
                        ...
                        {
                            int _event = (int)chip.counter[CHAN2];
                            PROCESS_CHANNEL(chip, CHAN2, _event, sum);
                        }
                    }
                    else
                    ...
                    {
                        int _event = (int)chip.counter[CHAN1];
                        PROCESS_CHANNEL(chip, CHAN1, _event, sum);
                    }
                }
                else
                ...
                {
                    int _event = chip.samplepos_whole;
                    PROCESS_SAMPLE(chip, samples, bufL, bufR, bufPtr, _event, sum);
                }
            }/*																
        ...
        //            {
        //                int _event = chip.samplepos_whole;
        //                int channel = unchecked((int)SAMPLE);
        //                if (chip.counter[CHAN1] < _event)
        //                {

        //                    _event = (int)chip.counter[CHAN1];
        //                    channel = CHAN1;
        //                }
        //                if (chip.counter[CHAN2] < _event)
        //                {

        //                    _event = (int)chip.counter[CHAN2];
        //                    channel = CHAN2;
        //                }
        //                if (chip.counter[CHAN3] < _event)
        //                {

        //                    _event = (int)chip.counter[CHAN3];
        //                    channel = CHAN3;
        //                }
        //                if (chip.counter[CHAN4] < _event)
        //                {

        //                    _event = (int)chip.counter[CHAN4];
        //                    channel = CHAN4;
        //                }
        //                if (channel == unchecked((int)SAMPLE))
        //                {
        //                    PROCESS_SAMPLE(chip, samples, bufL, bufR, bufPtr, _event, sum);
        //                }
        //                else
        ...
        private int device_start_pokey(byte ChipID, int clock)
        {
            ...
            chip.samplerate_24_8 = (int)((clock << 8) / sample_rate);
        ...
        /*READ8_HANDLER( quad_pokey_r )
        {
        	static final char *final devname[4] = { "pokey1", "pokey2", "pokey3", "pokey4" };
        ...
        private void pokey_w(byte ChipID, int offset, byte data)
        {
            ...
            switch (offset & 15)
            {
                ...
                case AUDCTL_C:
                    ...
                    p.clockmult = (int)((p.AUDCTL & CLK_15KHZ) != 0 ? DIV_15 : DIV_64);
                    break;
                ...
                case SKREST_C:
                    ...
                    p.SKSTAT &= (byte)~(SK_FRAME | SK_OVERRUN | SK_KBERR);
                    break;
                ...
                case SKCTL_C:
                    ...
                    if ((data & SK_RESET) == 0)
                    {
                        pokey_w(ChipID, IRQEN_C, (byte) 0);
                        pokey_w(ChipID, SKREST_C,(byte) 0);
                    }
                    break;
            }
            ...
            if ((ch_mask & (1 << CHAN1)) != 0)
            {
                ...
                p.volume[CHAN1] = (int)((p.AUDC[CHAN1] & VOLUME_MASK) * POKEY_DEFAULT_GAIN);
            ...
            if ((ch_mask & (1 << CHAN2)) != 0)
            {
                ...
                p.volume[CHAN2] = (int)((p.AUDC[CHAN2] & VOLUME_MASK) * POKEY_DEFAULT_GAIN);
            ...
            if ((ch_mask & (1 << CHAN3)) != 0)
            {
                ...
                p.volume[CHAN3] = (int)((p.AUDC[CHAN3] & VOLUME_MASK) * POKEY_DEFAULT_GAIN);
            ...
            if ((ch_mask & (1 << CHAN4)) != 0)
            {
                ...
                p.volume[CHAN4] = (int)((p.AUDC[CHAN4] & VOLUME_MASK) * POKEY_DEFAULT_GAIN);
