package MDSound;
	...
	public class x1_010 extends Instrument
	{
		@Override public String getName() { return "X1-010"; } 
		@Override public String getShortName() { return "X1-010"; } 
		...
		{
			return (int)device_start_x1_010(ChipID, (int)16000000);
		}

		@Override public int Start(byte ChipID, int clock, int ClockValue, Object... option)
		{
			return (int)device_start_x1_010(ChipID, (int)ClockValue);
		}
		...
		***************************************************************************/
		...
		private class x1_010_state
		{
			...
			public int rate;                               // Output sampling rate (Hz)
														   ...
														   //final (int)8 *region;					// region name
			public int ROMSize;
			public byte[] rom;
			public int sound_enable;                       // sound output enable/disable
			public byte[] reg = new byte[0x2000];              // X1-010 Register & wave form area
															   //	(int)8	HI_WORD_BUF[0x2000];			// X1-010 16bit access ram check avoidance work
			public int[] smp_offset = new int[SETA_NUM_CHANNELS];
			public int[] env_offset = new int[SETA_NUM_CHANNELS];
		...
		private x1_010_state[] X1010Data = new x1_010_state[] { new x1_010_state(), new x1_010_state() };// MAX_CHIPS];
		...
		*/
		...
		private void seta_update(byte ChipID, int[][] outputs, int samples)
		{
			...
			byte data;
			int env;
			int smp_offs, smp_step, env_offs, env_step, delta;
			...
			for (ch = 0; ch < SETA_NUM_CHANNELS; ch++)
			{
				...
				{       // Key On
					...
					{ // PCM sampling
						...
						if (freq == 0) freq = 4;
						smp_step = (int)((float)info.base_clock / 8192.0f
						...
						for (i = 0; i < samples; i++)
						{
							...
							data = (byte)info.rom[start + delta];
					...
					{ // Wave form
						...
						smp_step = (int)((float)info.base_clock / 128.0 / 1024.0 / 4.0 * freq * (1 << FREQ_BASE_BITS) / (float)info.rate + 0.5f);
						...
						for (i = 0; i < samples; i++)
						{
							...
							data = (byte)info.reg[start + ((smp_offs >> FREQ_BASE_BITS) & 0x7f)];
							bufL[i] += (data * volL / 256);
							bufR[i] += (data * volR / 256);
							smp_offs += smp_step;
							env_offs += env_step;
		...
		{
			int i;
			//final x1_010_interface *intf = (final x1_010_interface *)device.static_config();
