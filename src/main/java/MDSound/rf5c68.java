...
public class rf5c68 extends Instrument
    {
        @Override public String getName() { return "RF5C68"; }
        @Override public String getShortName() { return "RF68"; }
        ...
        {
            return (int)device_start_rf5c68(ChipID, (int)ClockValue);
        }
        ...
        //void rf5c68_write_ram(byte ChipID, offs_t DataStart, offs_t DataLength, final byte* RAMData);
        //void rf5c68_set_mute_mask(byte ChipID, (int)32 MuteMask);
        ...
        private final int STEAM_STEP = 0x800;
        ...
        {
            //sound_stream *		stream;
            public pcm_channel[] chan = new pcm_channel[] {
        ...
        public rf5c68_state[] RF5C68Data = new rf5c68_state[] { new rf5c68_state(), new rf5c68_state() };

        /*INLINE rf5c68_state *get_safe_token(final device_config *device)
        {
            assert(device != NULL);
        ...
        private void memstream_sample_check(rf5c68_state chip, int addr, int Speed)
        {
            mem_stream ms = chip.memstrm;
            int SmplSpd;

            SmplSpd = (int)((Speed >= 0x0800) ? (Speed >> 11) : 1);
            if (addr >= ms.CurAddr)
            {
            ...
            else
            {
                ...
                if (ms.CurAddr - addr <= SmplSpd * 5)
                {
                    ...
                    else
                    {
                        ...
                        System.arraycopy(ms.MemPnt, (ms.CurAddr - ms.BaseAddr), chip.data, ms.CurAddr, SmplSpd * 4);
                        ms.CurAddr += SmplSpd * 4;
                    }
                }
            }
        ...
        private void rf5c68_update(byte ChipID, int[][] outputs, int samples)
        {
            ...
            for (i = 0; i < NUM_CHANNELS; i++)
            {
                ...
                if (chan.enable != 0)// && chan.Muted == 0)
                {
                    ...
                    for (j = 0; j < samples; j++)
                    {
                        ...
                        //System.err.printf("Ch:{0} L:{1} R:{2}", i, outputs[0][j], outputs[1][j]);
                    }
                }
            }
            ...
            {
                ms.CurStep += (int)(STEAM_STEP * samples);
                if (ms.CurStep >= 0x0800)  // 1 << 11
                {
                    ...
                    System.arraycopy(ms.MemPnt, (ms.CurAddr - ms.BaseAddr), chip.data, ms.CurAddr, i);
                    ms.CurAddr += (int)i;
                }
            }
        ...
        {
            //final rf5c68_interface* intf = (final rf5c68_interface*)device.baseconfig().static_config();
        ...
        public void rf5c68_w(byte ChipID, int offset, byte data)
        {
            ...
            switch (offset)
            {
                ...
                case 0x02:  /* FDL */
                    chan.step = (int)((chan.step & 0xff00) | (data & 0x00ff));
                    break;

                case 0x03:  /* FDH */
                    chan.step = (int)((chan.step & 0x00ff) | ((data << 8) & 0xff00));
                    break;

                case 0x04:  /* LSL */
                    chan.loopst = (int)((chan.loopst & 0xff00) | (data & 0x00ff));
                    break;

                case 0x05:  /* LSH */
                    chan.loopst = (int)((chan.loopst & 0x00ff) | ((data << 8) & 0xff00));
                    break;

                case 0x06:  /* ST */
                    chan.start = data;
                    if (chan.enable == 0)
                        chan.addr = (int)(chan.start << (8 + 11));
                    break;
                ...
                case 0x08:  /* channel on/off reg */
                    for (i = 0; i < 8; i++)
                    {
                        ...
                        if (chip.chan[i].enable == 0)
                            chip.chan[i].addr = (int)(chip.chan[i].start << (8 + 11));
                    }
                    break;
            }
        }
        ...
        private void rf5c68_mem_stream_flush(rf5c68_state chip)
        {
            ...
            System.arraycopy(ms.MemPnt, (ms.CurAddr - ms.BaseAddr), chip.data, ms.CurAddr, ms.EndAddr - ms.CurAddr);
        ...
        private void rf5c68_write_ram(byte ChipID, int DataStart, int DataLength, byte[] RAMData)
        {
            ...
            ms.BaseAddr = (int)DataStart;
            ms.CurAddr = ms.BaseAddr;
            ms.EndAddr = (int)(ms.BaseAddr + DataLength);
            ms.CurStep = 0x0000;
            ms.MemPnt = RAMData;
            ...
            System.arraycopy(ms.MemPnt, (ms.CurAddr - ms.BaseAddr), chip.data, ms.CurAddr, BytCnt);
            ms.CurAddr += BytCnt;
