...
import java.util.List;
    ...
    public class SN76496 extends Instrument
    {
        @Override public String getName() {  return "SN76496";  }
        @Override public String getShortName() {  return "DCSGmame";  }

        private List<sn76496_state> chips = new ArrayList<sn76496_state>();

        @Override public void Reset(byte ChipID)
        {
            if (chips.get(ChipID) == null) return;
            sn76496_reset(chips.get(ChipID));
        }

        @Override public int Start(byte ChipID, int clock)
        {
            sn76496_state chip = new sn76496_state();
            int i = (int)sn76496_start(chip, (int)3579545, 0, 0, 0, 0, 0, 0);
        ...
        @Override public int Start(byte ChipID, int clock, int ClockValue, Object... option)
        {
            ...
            sn76496_state chip = new sn76496_state();
            int i = (int)sn76496_start(chip, (int)ClockValue, shiftreg, noisetaps, negate, stereo, divider, freq0);
        ...
        {
            if (chips.get(ChipID) == null) return 0;
            sn76496_stereo_w(chips.get(ChipID), adr, (byte)data);//adr 未使用
            return 0;
        }
        ...
        //private final int MAX_OUTPUT = 0x7fff;
        private final int MAX_OUTPUT = 0x8000;
        ...
        public class sn76496_state
        {
            ...
            public int[] VolTable = new int[16]; /* volume table (for 4-bit to db conversion)*/
            public int[] Register = new int[8];  /* registers */
            public int LastRegister; /* last register written */
            public int[] Volume = new int[4];    /* db volume of voice 0-2 and noise */
            public int RNG;         /* noise generator LFSR*/
            public int ClockDivider; /* clock divider */
            public int CurrentClock;
            public int FeedbackMask; /* mask for feedback */
            public int WhitenoiseTap1;   /* mask for white noise tap 1 (higher one, usually bit 14) */
            public int WhitenoiseTap2;   /* mask for white noise tap 2 (lower one, usually bit 13)*/
            public int Negate;       /* output negate flag */
            public int Stereo;       /* whether we're dealing with stereo or not */
            public int StereoMask;   /* the stereo output mask */
            public int[] Period = new int[4];    /* Length of 1/2 of waveform */
            public int[] Count = new int[4];     /* Position within the waveform */
            public int[] Output = new int[4];    /* 1-bit output of each channel, pre-volume */
            public int CyclestoREADY;/* number of cycles until the READY line goes active */
            public int Freq0IsMax;   /* flag for if frequency zero acts as if it is one more than max (0x3ff+1) or if it acts like 0 */
            public int[] MuteMsk = new int[4];
...
// #endif
        ...
        private long sn76496_start(sn76496_state chip, int clock, int shiftregwidth, int noisetaps,
                                        int negate, int stereo, int clockdivider, int freq0)
        {
            ...
            return (long)generic_start(chip, clock, 1 << (shiftregwidth - 1), ntap[0], ntap[1],
        ...
        {
            FNumLimit = (int)((clock / (clockdiv != 0 ? 2.0 : 16.0)) / sample_rate);

            return;
        }

        private void sn76496_set_mutemask(sn76496_state chip, int MuteMask)
        {
            sn76496_state R = (sn76496_state)chip;
            byte CurChn;

            for (CurChn = 0; CurChn < 4; CurChn++)
                R.MuteMsk[CurChn] = (MuteMask & (1 << CurChn)) != 0 ? 0 : ~0;
