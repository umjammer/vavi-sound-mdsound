...
import java.util.function.Consumer;
    ...
    public class ym3812 extends Instrument
    {
        @Override public String getName() { return "YM3812"; }
        @Override public String getShortName() { return "OPL2"; }
        ...
        private final int DefaultYM3812ClockValue = 3579545;
        ...
        {
            return (int)device_start_ym3812(ChipID, (int)ClockValue);
        }
        ...
        //        void ym3812_stream_update((int)8 ChipID, stream_sample_t** outputs, int samples);
        //        int device_start_ym3812((int)8 ChipID, int clock);
        //        void device_stop_ym3812((int)8 ChipID);
        //        void device_reset_ym3812((int)8 ChipID);

        //        (int)8 ym3812_r((int)8 ChipID, offs_t offset);
        //        void ym3812_w((int)8 ChipID, offs_t offset, (int)8 data);

        //        (int)8 ym3812_status_port_r((int)8 ChipID, offs_t offset);
        //        (int)8 ym3812_read_port_r((int)8 ChipID, offs_t offset);
        //        void ym3812_control_port_w((int)8 ChipID, offs_t offset, (int)8 data);
        //        void ym3812_write_port_w((int)8 ChipID, offs_t offset, (int)8 data);

        //        void ym3812_set_emu_core((int)8 Emulator);
        //        void ym3812_set_mute_mask((int)8 ChipID, (int)32 MuteMask);
        ...
        ******************************************************************************/
        ...
        private final int EC_DBOPL = 0x00;	// DosBox OPL (AdLibEmu)
        //# ifdef ENABLE_ALL_CORES
        private final int EC_MAME = 0x01;	// YM3826 core from MAME
        //#endif
        ...
        private ym3812_state[] YM3812Data = new ym3812_state[] { new ym3812_state(), new ym3812_state() };

        /*INLINE ym3812_state *get_safe_token(final device_config *device)
        {
            assert(device != NULL);
        ...
        public interface ADL_UPDATEHANDLER extends Consumer<ym3812_state>{};
        ...
        private int device_start_ym3812(byte ChipID, int clock)
        {
            ...
            switch (EMU_CORE)
            {
                ...
                case EC_DBOPL:
                    ...
                    info.chip = ADLIBEMU_init((int)(clock & 0x7FFFFFFF), (int)rate, this::_stream_update, info);
                    break;
            }
        ...
        private byte ym3812_r(byte ChipID, int offset)
        {
            ...
            switch (EMU_CORE)
            {
                ...
                case EC_DBOPL:
                    ...
                    return (byte)ADLIBEMU_reg_read(info.chip, (int)(offset & 0x01));
                default:
                    return 0x00;
            }
        }
        ...
        private void ym3812_w(byte ChipID, int offset, byte data)
        {
            ...
            switch (EMU_CORE)
            {
                ...
                case EC_DBOPL:
                    ...
                    ADLIBEMU_writeIO(info.chip, (int)(offset & 1), data);
                    break;
            }
        }
        ...
        //void* ADLIBEMU(init)((int)32 clock, (int)32 samplerate,
        //                     ADL_UPDATEHANDLER UpdateHandler, void* param);
        //void ADLIBEMU(stop)(void* chip);
        //void ADLIBEMU(reset)(void* chip);

        //void ADLIBEMU(writeIO)(void* chip, (int)32 addr, (int)8 val);
        //void ADLIBEMU(getsample)(void* chip, int** sndptr, int numsamples);

        //(int)32 ADLIBEMU(reg_read)(void* chip, (int)32 port);
        //void ADLIBEMU(write_index)(void* chip, (int)32 port, (int)8 val);

        //void ADLIBEMU(set_mute_mask)(void* chip, (int)32 MuteMask);
 ...
 */
        ...
        */
        ...
        //#else
        private final int NUM_CHANNELS = 9;
        //#endif

        private final int MAXOPERATORS = (NUM_CHANNELS * 2);


        private final double FL05 = ((double)0.5);
        private final double FL2 = ((double)2.0);
        private final double PI = ((double)3.1415926535897932384626433832795);


        private final static int FIXEDPT = 0x10000;        // fixed-point calculations using 16+16
        private final int FIXEDPT_LFO = 0x1000000;  // fixed-point calculations using 8+24

        private final int WAVEPREC = 1024;      // waveform precision (10 bits)
        ...
        private final int BLOCKBUF_SIZE = 512;


        // vibrato constants
        private final int VIBTAB_SIZE = 8;
        private final double VIBFAC = 70 / 50000;       // no braces, integer mul/div

        // tremolo constants and table
        private final int TREMTAB_SIZE = 53;
        private final double TREM_FREQ = ((double)(3.7));           // tremolo at 3.7hz
        ...
        */
        ...
        {//operator_struct
            ...
            //#if defined(OPLTYPE_IS_OPL3)
            //boolean is_4op,is_4op_attached;	// base of a 4op channel/part of a 4op channel
        ...
        // enable an operator
        //static void enable_operator(OPL_DATA* chip, int regbase, op_type* op_pt, int act_type);

        // functions to change parameters of an operator
        //static void change_frequency(OPL_DATA* chip, int chanbase, int regbase, op_type* op_pt);

        //static void change_attackrate(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_decayrate(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_releaserate(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_sustainlevel(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_waveform(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_keepsustain(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_vibrato(OPL_DATA* chip, int regbase, op_type* op_pt);
        //static void change_feedback(OPL_DATA* chip, int chanbase, op_type* op_pt);

        // general functions
        /*void* adlib_init(int clock, int samplerate);
        void adlib_writeIO(void *chip, int addr, byte val);
        void adlib_write(void *chip, int idx, byte val);
        //void adlib_getsample(short* sndptr, int numsamples);
        void adlib_getsample(void *chip, int** sndptr, int numsamples);
        int adlib_reg_read(void *chip, int port);
        void adlib_write_index(void *chip, int port, byte val);*/
        //static void adlib_write(void *chip, int idx, byte val);
        ...
        private byte[] modulatorbase = new byte[] {
            0,1,2,
            8,9,10,
            16,17,18
        };
        ...
        //#if defined(OPLTYPE_IS_OPL3)
        //static final byte regbase2modop[44] = {
        ...
        //};
        //static final byte regbase2op[44] = {
        ...
        //};
        //#else
        private byte[] regbase2modop = new byte[] {
            0,1,2,0,1,2,0,0,3,4,5,3,4,5,0,0,6,7,8,6,7,8
        };
        private byte[] regbase2op = new byte[] {
        ...
        private double[] attackconst = new double[] {
            (double)(1/2.82624),
            (double)(1/2.25280),
            (double)(1/1.88416),
            (double)(1/1.59744)
        ...
        private double[] decrelconst = new double[]{
            (double)(1/39.28064),
            (double)(1/31.41608),
            (double)(1/26.17344),
            (double)(1/22.44608)
        ...
        private void operator_advance_drums(OPL_DATA chip, op_type op_pt1, int vib1, op_type op_pt2, int vib2, op_type op_pt3, int vib3)
        {
            ...
            int noisebit = (int)(rand.nextInt() & 1);

            int snare_phase_bit = (((int)((op_pt1.tcount / FIXEDPT) / 0x100)) & 1);

            //Hihat
            int inttm = (int)((phasebit << 8) | (int)(0x34 << (int)(phasebit ^ (noisebit << 1))));
        ...
        private void operator_output(op_type op_pt, int modulator, int trem)
        {
            if (op_pt.op_state != OF_TYPE_OFF)
            {
                ...
                op_pt.cval = (int)(op_pt.step_amp * op_pt.vol * op_pt.cur_wform[op_pt.cur_wform_ptr + (i & op_pt.cur_wmask)] * trem / 16.0);
            }
        }
        ...
        private optype_fptr[] opfuncs = new optype_fptr[]{
            ym3812::operator_attack,
            ym3812::operator_decay,
            ym3812::operator_release,
            ym3812::operator_sustain,	// sustain phase (keeping level)
            ym3812::operator_release,	// sustain_nokeep phase (release-style)
            ym3812::operator_off
        };

        private byte[] step_skip_mask = new byte[] { (byte) 0xff, (byte) 0xfe, (byte) 0xee, (byte) 0xba, (byte) 0xaa };

        private void change_attackrate(OPL_DATA chip, int regbase, op_type op_pt)
        {
        ...
        private void change_decayrate(OPL_DATA chip, int regbase, op_type op_pt)
        {
        ...
        private void change_releaserate(OPL_DATA chip, int regbase, op_type op_pt)
        {
        ...
        private void change_sustainlevel(OPL_DATA chip, int regbase, op_type op_pt)
        {
        ...
        private void change_waveform(OPL_DATA chip, int regbase, op_type op_pt)
        {
            //#if defined(OPLTYPE_IS_OPL3)
        ...
        private void change_keepsustain(OPL_DATA chip, int regbase, op_type op_pt)
        {
        ...
        private void change_vibrato(OPL_DATA chip, int regbase, op_type op_pt)
        {
        ...
        private void change_feedback(OPL_DATA chip, int chanbase, op_type op_pt)
        {
        ...
        private void change_frequency(OPL_DATA chip, int chanbase, int regbase, op_type op_pt)
        {
            int frn;
            int oct;
            int note_sel;
        ...
        private void enable_operator(OPL_DATA chip, int regbase, op_type op_pt, int act_type)
        {
        ...
        private void disable_operator(op_type op_pt, int act_type)
        {
        ...
        private OPL_DATA ADLIBEMU_init(int clock, int samplerate, ADL_UPDATEHANDLER UpdateHandler, ym3812_state param)
        {
            OPL_DATA OPL;
            //op_type* op;
            ...
            OPL.generator_add = (int)(INTFREQU(OPL.chip_clock) * FIXEDPT / OPL.int_samplerate);
        ...
        // #endif
            ...
            OPL.vibtab_add = (int)(VIBTAB_SIZE * FIXEDPT_LFO / 8192 * INTFREQU(OPL.chip_clock) / OPL.int_samplerate);
            OPL.vibtab_pos = 0;
        ...
        private void ADLIBEMU_writeIO(OPL_DATA chip, int addr, byte val)
        {
            OPL_DATA OPL = (OPL_DATA)chip;
        ...
        private void adlib_write(OPL_DATA chip, int idx, byte val)
        {
            OPL_DATA OPL = (OPL_DATA)chip;
        ...
        private int ADLIBEMU_reg_read(OPL_DATA chip, int port)
        {
            OPL_DATA OPL = (OPL_DATA)chip;
            ...
            if ((port & 1) == 0)
            {
                return (int)(OPL.status | 6);
            }
            return 0xff;
            //#endif
        }

        private void ADLIBEMU_write_index(OPL_DATA chip, int port, byte val)
        {
            OPL_DATA OPL = (OPL_DATA)chip;
        ...
        //void adlib_getsample(short* sndptr, int numsamples)
        private void ADLIBEMU_getsample(OPL_DATA chip, int[][] sndptr, int numsamples)
        {
            OPL_DATA OPL = (OPL_DATA)chip;
        ...
        private void ADLIBEMU_set_mute_mask(OPL_DATA chip, int MuteMask)
        {
            OPL_DATA OPL = (OPL_DATA)chip;
