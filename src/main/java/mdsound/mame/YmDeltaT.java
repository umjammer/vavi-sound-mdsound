...
public class YmDeltaT {
    ...
    public void reset2610(double freqBase, int[] outDelta) {
        ...
        this.portShift = 8; // always 8bits shift
    ...
    public int readAdpcm() {
        ...
        if ((this.portState & 0xe0) == 0x20) {
            ...
            if (this.nowAddr != (this.end << 1)) {
                ...
                //logger.log(Level.TRACE, "YM Delta-T memory read  $%08x, v=$%02x\n", this.now_addr >> 1, v);
    ...
    public void writeAdpcm(int r, int v) {
        if (r >= 0x10) return;
        this.reg[this.regPtr + r] = v; // stock data
        ...
        case 0x03: // Start Address H
            this.start = (this.reg[this.regPtr + 0x3] * 0x0100 | this.reg[this.regPtr + 0x2]) << (this.portShift - this.dromPortShift);
            //logger.log(Level.TRACE, "DeltaT start: 02=%2x 03=%2x addr=%8x\n",this.reg[0x2], this.reg[0x3],this.start );
            break;
        case 0x04: // Stop Address L
        case 0x05: // Stop Address H
            this.end = (this.reg[this.regPtr + 0x5] * 0x0100 | this.reg[this.regPtr + 0x4]) << (this.portShift - this.dromPortShift);
            this.end += (1 << (this.portShift - this.dromPortShift)) - 1;
            //logger.log(Level.TRACE, "DeltaT end  : 04=%2x 05=%2x addr=%8x\n",this.reg[0x4], this.reg[0x5],this.end   );
            break;
        ...
        case 0x08: // ADPCM data
            ...
            if ((this.portState & 0xe0) == 0x60) {
                ...
                //logger.log(Level.TRACE, "YM Delta-T memory write $%08x, v=$%02x\n", this.now_addr >> 1, v);
        ...
        case 0x0a: // DELTA-N H */
            ...
            //logger.log(Level.TRACE, "DeltaT deltaN:09=%2x 0a=%2x\n",this.reg[0x9], this.reg[0xa]);
            break;
        case 0x0b: { // Output level control (volume, linear)
            ...
            //         v     *     ((1<<23)>>8)        >>  15;
            //         v     *     (1<<15)             >>  15;
            //logger.log(Level.TRACE, "DeltaT vol = %2x\n",v&0xff);
            if (oldvol != 0) {
        ...
        case 0x0d: // Limit Address H
            this.limit = (this.reg[this.regPtr + 0xd] * 0x0100 | this.reg[this.regPtr + 0xc]) << (this.portShift - this.dromPortShift);
            //logger.log(Level.TRACE, "DeltaT limit: 0c=%2x 0d=%2x addr=%8x\n",this.reg[0xc], this.reg[0xd],this.limit );
            break;
        }
    }
    ...
    public void resetAdpcm(int pan, int emulationMode) {
        ...
        this.portState = (emulationMode == EMULATION_MODE_YM2610) ? 0x20 : 0;
        this.control2 = (emulationMode == EMULATION_MODE_YM2610) ? 0x01 : 0; // default setting depends on the emulation mode. MSX demo called "facdemo_4" doesn't setup control2 register at all and still works
    ...
    public void postLoad(int[] regs, int regPtr) {
        int r;
