...
//
    public class OPM extends Timer
    {
        ...
        private int lfo_count_;
        private int lfo_count_diff_;
        private int lfo_step_;
        private int lfo_count_prev_;
        ...
        private void LFO()
        {
            if (lfowaveform != 3)
            {
                //        if ((lfo_count_ ^ lfo_count_prev_) & ~((1 << 15) - 1))
                {
                    ...
                    chip.SetPML((int)(pmtable[lfowaveform][c] * pmd / 128 + 0x80));
                    chip.SetAML((int)(amtable[lfowaveform][c] * amd / 128));
                }
            }
            else
            {
                if (((lfo_count_ ^ lfo_count_prev_) & ~((1 << 17) - 1))!=0)
                {
                    ...
                    chip.SetPML((int)((c - 0x80) * pmd / 128 + 0x80));
                    chip.SetAML((int)(c * amd / 128));
                }
            }
