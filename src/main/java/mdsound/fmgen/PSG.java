                ...
                if ((r7 & 0x38) == 0) {
                    ...
                    for (int i = 0; i < nsamples; i++) {
                        ...
                        for (int j = 0; j < (1 << overSampling); j++) {
                            ...
                            x = (((sCount[0] >> (toneShift + overSampling)) & chenable[0]) | (nenable[0] & noise)) - 1;     // 0 or -1
                            sample += (olevel[0] + x) ^ x;
                            sCount[0] += speriod[0];

                            y = (((sCount[1] >> (toneShift + overSampling)) & chenable[1]) | (nenable[1] & noise)) - 1;
                            sample += (olevel[1] + y) ^ y;
                            sCount[1] += speriod[1];

                            z = (((sCount[2] >> (toneShift + overSampling)) & chenable[2]) | (nenable[2] & noise)) - 1;
                            sample += (olevel[2] + z) ^ z;
                            sCount[2] += speriod[2];
