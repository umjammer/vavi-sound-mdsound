...
public class PPZ8 extends Instrument.BaseInstrument {
    @Override
    ...
    public static class PPZ8Status {
         ...
         * @param dx PCMの音色番号
         */
        private void playPCM(byte al, int dx) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: PlayPCM: ch:%d @:%d", al, dx));
// #endif
         ...
         * @param al PCMチャンネル(0-7)
         */
        private void stopPCM(byte al) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: StopPCM: ch:%d", al));
// #endif
...
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: LoadPCM: bank:%d mode:%d", bank, mode));
// #endif
         ...
         * @param al
         */
        private void readStatus(byte al) {
            switch (al) {
            case 0xd:
// #if DEBUG
...
// #endif
         ...
         */
        private void setVolume(byte al, int dx) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: SetVolume: Ch:%d vol:%d", al, dx));
// #endif
            chWk[al].volume = dx;
        }
         ...
         */
        private void setFrequency(byte al, int dx, int cx) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: SetFrequency: 0x%8x", dx * 0x10000 + cx));
// #endif
         ...
         */
        private void setLoopPoint(byte al, int lpStOfsDX, int lpStOfsCX, int lpEdOfsDI, int lpEdOfsSI) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: SetLoopPoint: St:0x%8x Ed:0x%8x"
...
// #endif
         ...
         * @param dx PAN(0~9)
         */
        private void setPan(byte al, int dx) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em:sSetPan: %d", dx));
// #endif
            chWk[al].pan = dx;
         ...
         * @param dx 元周波数
         */
        private void setSrcFrequency(byte al, int dx) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: setSrcFrequency: %d", dx));
// #endif
...
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: SetAllVolume: %d", vol));
// #endif
         ...
         */
        private void setAdpcmEmu(byte al) {
// #if DEBUG
            Log.writeLine(LogLevel.TRACE, String.format("ppz8em: setAdpcmEmu: %d", al));
// #endif
            adpcmEmu = al;
        }
         ...
         */
        private void setReleaseFlag(int v) {
            //なにもしない
        }
        ...
        private int convertPviAdpcmToPziPcm(byte bank) {
            ...
            pcmData[bank] = Common.toByteArray(o);
