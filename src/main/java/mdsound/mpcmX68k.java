...
public class mpcmX68k extends Instrument
    {
        @Override public String getName()  { return "mpcmX68k"; }
        @Override public String getShortName() { return "mpx"; }
        ...
        protected enum TYPE
        {
            ...
            int v;
            TYPE(int v) { this.v = v; }
            static TYPE valueOf(int v) { return Arrays.stream(values()).filter(e -> e.v == v).findFirst().get(); }
        }
        ...
        {
            m[chipID].work = new CHWORK[(int)VOICE.MAX.ordinal()];
            for(int i=0;i< (int)VOICE.MAX.ordinal(); i++)
            {
                m[chipID].work[i] = new CHWORK();
            }
            m[chipID].DiffTable = new int[(int)TBL.DIFF.ordinal()];
        }
        ...
        public boolean Initialize(int chipID, int base_, float sampling_rate)
        {
        ...
        public void Reset_(int chipID)
        {
            ...
            for (int ch = 0; ch < (int)VOICE.MAX.ordinal(); ch++)
            {
                KeyOff(chipID, ch);
                SetVol(chipID, ch, 64);
        ...
        public void KeyOn(int chipID, int ch)
        {
            if (ch == 0xff)
            {
                for (int i = 0; i < (int)VOICE.MAX.ordinal(); i++) KeyOn(chipID, i);
            }
            else
            {
        ...
        public void KeyOff(int chipID, int ch)
        {
            if (ch == 0xff)
            {
                for (int i = 0; i < (int)VOICE.MAX.ordinal(); i++) KeyOff(chipID, i);
            }
            else
            {
        ...
        public boolean SetPcm(int chipID, int ch, SETPCM ptr)
        {
            if (ch == 0xff)
            {
                for (int i = 0; i < (int)VOICE.MAX.ordinal(); i++) SetPcm(chipID, i, ptr);
            }
            else
            {
                KeyOff(chipID, ch);
                ...
                m[chipID].work[ch].orig = (int)(ptr.orig << 6);
        ...
        public void SetPitch(int chipID, int ch, int note)
        {
            if (ch == 0xff)
            {
                for (int i = 0; i < (int)VOICE.MAX.ordinal(); i++) SetPitch(chipID, i, note);
            }
            else
            {
                int orig = m[chipID].work[ch].orig;
                int pitch = 0x10000;
                ...
                if (orig > 0x1fc0)
                {
                    m[chipID].work[ch].pitch = (int)(0x10000 * m[chipID].base_);
                    return;
                }
                ...
                m[chipID].work[ch].pitch = (int)(pitch * m[chipID].base_);
            }
        }
        ...
        public void SetVol(int chipID, int ch, int vol)
        {
            if (ch == 0xff)
            {
                for (int i = 0; i < (int)VOICE.MAX.ordinal(); i++) SetVol(chipID, i, vol);
            }
            else
            {
                vol &= 0x7f;
        ...
        public void SetPan(int chipID, int ch, int pan)
        {
            if (ch == 0xff)
            {
                for (int i = 0; i < (int)VOICE.MAX.ordinal(); i++) SetPan(chipID, i, pan);
            }
            else
            {
                if (pan < 0x80)
        ...
        public void SetVolTable(int chipID, int sel, int[] tbl)
        {
            if (sel == 1)
            {
                // 16
        ...
        public void Update_(int chipID, int[][] _buffer, int _count)
        {
            ...
            for (int ch = 0; ch < (int)VOICE.MAX.ordinal(); ch++)
            {
                boolean mute = ((int)m[chipID].mask & (1 << ch)) != 0;
                buf_ptr = 0;

                byte[] ptr_buf = m[chipID].work[ch].adrs_buf;
                int ptr_ptr = m[chipID].work[ch].adrs_ptr;
                long ofst = m[chipID].work[ch].pos;
                int pitch = m[chipID].work[ch].pitch;
                for (int bufsize = 0; (bufsize < _count) && (m[chipID].work[ch].enable); bufsize++)
                {
                    sample = 0;
                    pos = ofst >> 16;

                    switch (TYPE.valueOf(m[chipID].work[ch].type))
                    {
                        case _NONE:
                            break;
                        case _ADPCM:
                    ...
                    if (!mute)
                    {
                        _buffer[0][buf_ptr] += (short)(sample * m[chipID].work[ch].lr[0]);
                        _buffer[1][buf_ptr] += (short)(sample * m[chipID].work[ch].lr[1]);
                        buf_ptr++;
                    }
