...
public class Sn76496 extends Instrument.BaseInstrument {
    @Override
     ...
     */
    public static class Sn76496State {
        ...
        public int cyclestoReady;
        /* flag for if frequency zero acts as if it is one more than max (0x3ff+1) or if it acts like 0 */
        public int freq0IsMax;
        public int[] muteMsk = new int[4];
        /* bit 7 - NGP Mode on/off, bit 0 - is 2nd NGP chip */
        public byte ngpFlags;
        /* Pointer to other Chip */
        public Sn76496State ngpChip2;

        private static Sn76496State lastChipInit = null;
...
// #endif
        ...
        void update(int[][] outputs, int samples) {
            int i;
            Sn76496State r2;
                ...
                if (this.ngpFlags == 0) {
                    for (i = 0; i < 4; i++) {
                        ...
                        if (this.period[i] > 1 || i == 3) {
                            ...
                            //out += (2 * this.Volume[i] - this.VolTable[5]) * ggst[0];
                            //out2 += (2 * this.Volume[i] - this.VolTable[5]) * ggst[1];
        ...
        private int startGeneric(int clock, int feedbackMask, int noiseTap1, int noiseTap2, int negate, int stereo, int clockdivider, int freq0) {
            ...
            if ((clock & 0x80000000) != 0 && lastChipInit != null) {
                ...
                Sn76496State chip2 = lastChipInit;
        ...
        private long start(int clock, int shiftRegWidth, int noiseTaps,
        ...
        private void limitFreq(int clock, int clockdiv, int sampleRate) {
