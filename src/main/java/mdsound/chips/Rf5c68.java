...
public class Rf5c68 {
    ...
    private byte[] data;
    private MemStream memstrm = new MemStream();
    ...
    public void write(int offset, byte data) {
        ...
        case 0x08: // channel on/off reg
            for (i = 0; i < 8; i++) {
                byte old = this.channels[i].enable;

                this.channels[i].enable = (byte) ((~data >> i) & 1);
    ...
    public void setMuteMask(int muteMask) {
        for (int c = 0; c < NUM_CHANNELS; c++)
            this.channels[c].muted = (byte) ((muteMask >> c) & 0x01);
    }

    public void setMute(int ch, int mute) {
        channels[ch].muted = (byte) mute;
    }
