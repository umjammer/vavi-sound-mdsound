...
public class Upd7759 {
    ...
    private int pos;                     /* current output sample position */
    private int step;                        /* step value per output sample */
    //attotime	clock_period;				/* clock period */
    //emu_timer *timer;						/* timer */

    /* I/O lines */
    private int fifo_in;                  /* last data written to the sound chip */
    private int reset;                        /* current state of the RESET line */
    private int start;                        /* current state of the START line */
    private int drq;                      /* current state of the DRQ line */
    //void (*drqcallback)(running_device *device, int param);			/* drq callback */
    //void (*drqcallback)(int param);			/* drq callback */

    /* internal state machine */
    private STATE state;                     /* current overall chip state */
    private int clocks_left;              /* number of clocks left in this state */
    private short nibbles_left;                /* number of ADPCM nibbles left to process */
    private byte repeat_count;             /* number of repeats remaining in current repeat block */
    private STATE post_drq_state;                /* state we will be in after the DRQ line is dropped */
    private int post_drq_clocks;          /* clocks that will be left after the DRQ line is dropped */
    private int req_sample;                   /* requested sample number */
    private int last_sample;              /* last sample number available */
    private int block_header;             /* header byte */
    private int sample_rate;              /* number of UPD clocks per ADPCM nibble */
    private int first_valid_header;           /* did we get our first valid header yet? */
    private int offset;                      /* current ROM offset */
    private int repeat_offset;               /* current ROM repeat offset */

    /* ADPCM processing */
    private int adpcm_state;               /* ADPCM state index */
    private int adpcm_data;                   /* current byte of ADPCM data */
    private short sample;                       /* current sample value */

    /* ROM access */
    private int romsize;
    private byte[] rom;                     /* pointer to ROM data or NULL for slave mode */
    private int romPtr = 0;
    private byte[] rombase;                 /* pointer to ROM data or NULL for slave mode */
    private int romoffset;                   /* ROM offset to make save/restore easier */
