...
//    }
    ...
    @Override
    public void update_envelopes(/* ref */ es550x_voice[] voice) {
    ...
    @Override
    public void check_for_end_forward(/* ref */ es550x_voice[] voice, long accum) {
        // are we past the end?
        if (accum > voice[0].end) {
            ...
            switch (voice[0].control & CONTROL_LOOPMASK) {
                ...
                case CONTROL_LPE:
                    accum = (voice[0].start + (accum - voice[0].end)) & m_address_acc_mask;
                    break;
    ...
    @Override
    public void check_for_end_reverse(/* ref */ es550x_voice[] voice, long accum) {
        // are we past the end?
        if (accum < voice[0].start) {
            ...
            switch (voice[0].control & CONTROL_LOOPMASK) {
                ...
                // uni-directional looping
                case (int) CONTROL_LPE:
                    accum = (voice[0].end - (voice[0].start - accum)) & m_address_acc_mask;
                    break;
    ...
    @Override
    public void generate_samples(int[][] outputs) {
...
//#endif
                ...
                int voice_channel = get_ca(voice[0].control);
     ...
     */
    private void reg_write_low(/* ref */ es550x_voice[] voice, int offset, short data, short mem_mask) {
        switch (offset) {
            case 0x00:  // CR
                voice[0].control |= 0xf000; // bit 15-12 always 1
                if ((mem_mask & 0x000000ff) != 0) {
//#if RAINE_CHECK
                    voice[0].control &= ~(CONTROL_STOPMASK | CONTROL_LOOPMASK | CONTROL_DIR);
//#else
//                voice[0].control &= unchecked((int)~0x00ff);
//#endif
                    voice[0].control |= data & 0x00ff;
                }
                if ((mem_mask & 0x0000ff00) != 0)
                    voice[0].control = (voice[0].control & ~0x0f00) | (data & 0x0f00);

                //logger.log(Level.TRACE, "%s:voice %d, control=%04x (raw=%04x & %04x)\n"
...
//logger.log(Level.TRACE, "%s:voice %d, loop end=%08x\n", machine().describe_context(), m_current_page & 0x1f, get_address_acc_res(voice.end));
            ...
            case 0x06: // K2
                if ((mem_mask & 0x0000_00ff) != 0)
                    voice[0].k2 = (voice[0].k2 & ~0x00f0) | (data & 0x00f0);
                if ((mem_mask & 0x0000_ff00) != 0)
                    voice[0].k2 = (voice[0].k2 & ~0xff00) | (data & 0xff00);
//logger.log(Level.TRACE, "%s:voice %d, K2=%03x\n", machine().describe_context(), m_current_page & 0x1f, voice.k2 >> FILTER_SHIFT);
                break;
...
//logger.log(Level.TRACE, "%s:voice %d, left vol=%02x\n", machine().describe_context(), m_current_page & 0x1f, voice.lvol);
            ...
            case 0x09: // RVOL
                if ((mem_mask & 0x0000_ff00) != 0)
                    voice[0].rVol = (voice[0].rVol & ~0xff) | ((data & 0xff00) >> 8);
//logger.log(Level.TRACE, "%s:voice %d, right vol=%02x\n", machine().describe_context(), m_current_page & 0x1f, voice.rvol);
                break;
...
//logger.log(Level.TRACE, "active voices=%d, sample_rate=%d\n", m_active_voices, m_sample_rate);
                }
                break;
    ...
    private void reg_write_high(/* ref */ es550x_voice[] voice, int offset, short data, short mem_mask) {
        switch (offset) {
            case 0x00: // CR
                voice[0].control |= 0xf000; // bit 15-12 always 1
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].control = (voice[0].control & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].control = (voice[0].control & ~0x0f00) | (data & 0x0f00);
...
//logger.log(Level.TRACE, "%s:voice %d, control=%04x (raw=%04x & %04x)\n", machine().describe_context(), m_current_page & 0x1f, voice.control, data, mem_mask);
            ...
            case 0x01: // O4(n-1)
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].o4n1 = (voice[0].o4n1 & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].o4n1 = (short) ((voice[0].o4n1 & ~0xff00) | (data & 0xff00));
//logger.log(Level.TRACE, "%s:voice %d, O4(n-1)=%04x\n", machine().describe_context(), m_current_page & 0x1f, voice[0].o4n1 & 0xffff);
                break;

            case 0x02: // O3(n-1)
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].o3n1 = (voice[0].o3n1 & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].o3n1 = (short) ((voice[0].o3n1 & ~0xff00) | (data & 0xff00));
//logger.log(Level.TRACE, "%s:voice %d, O3(n-1)=%04x\n", machine().describe_context(), m_current_page & 0x1f, voice[0].o3n1 & 0xffff);
                break;

            case 0x03: // O3(n-2)
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].o3n2 = (voice[0].o3n2 & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].o3n2 = (short) ((voice[0].o3n2 & ~0xff00) | (data & 0xff00));
//logger.log(Level.TRACE, "%s:voice %d, O3(n-2)=%04x\n", machine().describe_context(), m_current_page & 0x1f, voice[0].o3n2 & 0xffff);
                break;

            case 0x04: // O2(n-1)
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].o2n1 = (voice[0].o2n1 & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].o2n1 = (short) ((voice[0].o2n1 & ~0xff00) | (data & 0xff00));
//logger.log(Level.TRACE, "%s:voice %d, O2(n-1)=%04x\n", machine().describe_context(), m_current_page & 0x1f, voice[0].o2n1 & 0xffff);
                break;

            case 0x05: // O2(n-2)
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].o2n2 = (voice[0].o2n2 & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].o2n2 = (short) ((voice[0].o2n2 & ~0xff00) | (data & 0xff00));
//logger.log(Level.TRACE, "%s:voice %d, O2(n-2)=%04x\n", machine().describe_context(), m_current_page & 0x1f, voice[0].o2n2 & 0xffff);
                break;

            case 0x06: // O1(n-1)
                if (((mem_mask & 0x0000_00ff) != 0))
                    voice[0].o1n1 = (voice[0].o1n1 & ~0x00ff) | (data & 0x00ff);
                if (((mem_mask & 0x0000_ff00) != 0))
                    voice[0].o1n1 = (short) ((voice[0].o1n1 & ~0xff00) | (data & 0xff00));
//logger.log(Level.TRACE, "%s:voice %d, O1(n-1)=%04x (accum=%08x)\n", machine().describe_context(), m_current_page & 0x1f, voice.o1n1 & 0xffff, get_address_acc_res(voice.accum));
                break;
...
//logger.log(Level.TRACE, "active voices=%d, sample_rate=%d\n", m_active_voices, m_sample_rate);
    ...
    private void reg_write_test(/* ref */ es550x_voice[] voice, int offset, short data, short mem_mask) {
        switch (offset) {
            case 0x00: // CH0L
            ...
            case 0x08: // SERMODE
                ...
                if (((mem_mask & 0x0000ff00) != 0))
                    m_mode = (short) ((m_mode & ~0xf800) | (data & 0xf800)); // MSB[4:0] (unknown purpose)
                if (((mem_mask & 0x000000ff) != 0))
                    m_mode = (short) ((m_mode & ~0x0007) | (data & 0x0007)); // SONY/BB, TEST, A/D
//logger.log(Level.TRACE, "%s: serial mode = %04x & %04x", machine().describe_context(), m_mode, mem_mask);
                break;
...
//logger.log(Level.TRACE, "%s:ES5505 write %02x/%02x = %04x & %04x\n", machine().describe_context(), m_current_page, offset, data, mem_mask);
     ...
     */
    private short reg_read_low(/* ref */ es550x_voice[] voice, int offset) {
        short result = 0;

        switch (offset) {
            case 0x00: // CR
                result = (short) (voice[0].control | 0xf000);
                break;

            case 0x01: // FC
                result = (short) get_address_acc_res(voice[0].freqCount, 1);
                break;

            case 0x02: // STRT (hi)
                result = (short) (get_address_acc_res(voice[0].start, 0) >>> 16);
                break;

            case 0x03: // STRT (lo)
                result = (short) get_address_acc_res(voice[0].start, 0);
                break;

            case 0x04: // END (hi)
                result = (short) (get_address_acc_res(voice[0].end, 0) >>> 16);
                break;

            case 0x05: // END (lo)
                result = (short) get_address_acc_res(voice[0].end, 0);
                break;

            case 0x06: // K2
                result = (short) voice[0].k2;
                break;

            case 0x07: // K1
                result = (short) voice[0].k1;
                break;

            case 0x08: // LVOL
                result = (short) (voice[0].lvol << 8);
                break;

            case 0x09: // RVOL
                result = (short) (voice[0].rVol << 8);
                break;

            case 0x0a: // ACC (hi)
                result = (short) (get_address_acc_res(voice[0].accum, 0) >>> 16);
                break;

            case 0x0b: // ACC (lo)
                result = (short) get_address_acc_res(voice[0].accum, 0);
                break;
    ...
    private short reg_read_high(/* ref */ es550x_voice[] voice, int offset) {
        short result = 0;
        ...
        switch (offset) {
            ...
            case 0x01:  // O4(n-1)
                result = (short) (voice[0].o4n1 & 0xffff);
                break;

            case 0x02:  // O3(n-1)
                result = (short) (voice[0].o3n1 & 0xffff);
                break;

            case 0x03:  // O3(n-2)
                result = (short) (voice[0].o3n2 & 0xffff);
                break;

            case 0x04:  // O2(n-1)
                result = (short) (voice[0].o2n1 & 0xffff);
                break;

            case 0x05:  // O2(n-2)
                result = (short) (voice[0].o2n2 & 0xffff);
                break;
            ...
            case 0x06:  // O1(n-1)
                ...
                result = (short) (voice[0].o1n1 & 0xffff);
                break;
    ...
    private short reg_read_test(/* ref */ es550x_voice[] voice, int offset) {
        short result = 0;
        ...
        switch (offset) {
            ...
            case 0x08:  // SERMODE
                result = (short) (m_mode | 0x7f8);
                break;

            case 0x09:  // PAR
                //if (!m_read_port_cb.isunset())
                //result = (short)(m_read_port_cb(0) & 0xffc0); // 10 bit, 15:6
                break;
    ...
    private short read(int offset) {
        ...
        //logger.log(Level.TRACE, "%04x (accum=%08x)\n", result, voice.accum);
